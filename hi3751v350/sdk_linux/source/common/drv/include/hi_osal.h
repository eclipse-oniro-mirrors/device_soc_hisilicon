...
typedef struct osal_hrtimer {
    void *timer;
    OSAL_HRTIMER_RESTART_E (*handler)(void *timer);
...
// cache
void osal_flush_dcache_area(const void *kvirt, unsigned long phys_addr, unsigned long size);
...
#define PROC_CMD_SINGEL_LENGTH_MAX 16
#define OSAL_BUILDVARIANT_NONE    0x0
#define OSAL_BUILDVARIANT_USER    0x1
#define OSAL_BUILDVARIANT_ENG     0x2
...
typedef struct osal_proc_entry_ {
    ...
    int (*write)(struct osal_proc_entry_ *entry, const char *buf, int count, long long *);
