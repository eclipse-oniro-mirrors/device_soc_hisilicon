...
osal_pmops *osal_get_pmops(const pm_basedev *pm_base)
{
...
static int osal_open(struct inode *inode, struct file *file)
{
    ...
    fileops_node = (osal_fileops_info *)kmalloc(sizeof(osal_fileops_info), GFP_KERNEL);
    if (fileops_node == NULL) {
        osal_printk("%s - fileops_node kmalloc failed!\n", __FUNCTION__);
        return -1;
    }

    ret = memset_s(fileops_node, sizeof(osal_fileops_info), 0, sizeof(osal_fileops_info));
    if (ret != 0) {
        return -1;
    }
...
int osal_dev_register(osal_dev *dev)
{
    ...
    media = &dev_node->media;

    if (dev->minor != 0) {
        media->minor = dev->minor;
    } else {
        media->minor = HIMEDIA_DYNAMIC_MINOR;
    }

    media->owner = THIS_MODULE;
    media->app_ops = &g_osal_fops;
    media->base_ops = &g_osal_pmops;
    media->name = dev->name;
    media->dev = dev;

    ret =  hi_drv_pm_register(media);
    if (ret != 0) {
        kfree(dev_node);
        return -1;
    }
...
void osal_dev_unregister(osal_dev *dev)
{
    ...
    hi_drv_pm_un_register(&dev_node->media);
