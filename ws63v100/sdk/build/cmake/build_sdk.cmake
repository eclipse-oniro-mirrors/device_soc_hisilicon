...
function(install_sdk_local src partern)

    string(REPLACE "/" "_" _name ${src})
    string(REPLACE "@" "_" _name ${_name})
    if (TARGET COPY${_name}_local)
        return()
    endif ()
    set(dest ${src})
    get_filename_component(FILE_NAME ${src} NAME)
    add_custom_target(COPY${_name}_local ALL
            COMMENT "--Installing ${src} => ${dest}"
            COMMAND ${Python3_EXECUTABLE} ${BUILD_UTILS} copy "${FILE_NAME}" "${dest}" ".srcrelease" '${partern}'
    )
    add_dependencies(COPY${_name}_local ${TARGET_NAME})

endfunction()

function(install_sdk_local_dest src dest  partern)
