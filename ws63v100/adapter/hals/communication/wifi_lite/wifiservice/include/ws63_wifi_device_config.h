...
typedef enum wifi_security_enum {
    WS63_WIFI_SEC_TYPE_INVALID = -1,         /*!< @if Eng Invalid security type.
                                            @else 无效安全类型. @endif */
    WS63_WIFI_SEC_TYPE_OPEN,                 /*!< @if Eng Open.
                                             @else Open。 @endif */
    WS63_WIFI_SEC_TYPE_WEP,                  /*!< @if Eng Wired Equivalent Privacy SHARED system (WEP-SHARED).
                                             @else WEP。 @endif */
    WS63_WIFI_SEC_TYPE_WPA2PSK,              /*!< @if Eng WPA2-Personal.
                                             @else WPA2 个人级。 @endif */
    WS63_WIFI_SEC_TYPE_WPA2_WPA_PSK_MIX,     /*!< @if Eng WPA-Personal and WPA2-Personal mixed.
                                             @else 个人级的WPA和WPA2混合。 @endif */
    WS63_WIFI_SEC_TYPE_WPAPSK,               /*!< @if Eng WPA-Personal.
                                             @else WPA 个人级。 @endif */
    WS63_WIFI_SEC_TYPE_WPA,                  /*!< @if Eng WPA-Enterprise.
                                             @else WPA 企业级。 @endif */
    WS63_WIFI_SEC_TYPE_WPA2,                 /*!< @if Eng WPA2-Enterprise.
                                             @else WPA2 企业级。 @endif */
    WS63_WIFI_SEC_TYPE_SAE,                  /*!< @if Eng Simultaneous Authentication of Equals (SAE).
                                             @else SAE(WPA3个人级)。 @endif */
    WS63_WIFI_SEC_TYPE_WPA3_WPA2_PSK_MIX,    /*!< @if Eng WPA2-Personal and WPA3-Personal mixed.
                                             @else 个人级的WPA2和WPA3混合。 @endif */
    WS63_WIFI_SEC_TYPE_WPA3,                 /*!< @if Eng WPA3-Enterprise.
                                             @else WPA3 企业级。 @endif */
    WS63_WIFI_SEC_TYPE_OWE,                  /*!< @if Eng Opportunistic wireless encryption(OWE).
                                             @else OWE。 @endif */
    WS63_WIFI_SEC_TYPE_WAPI_PSK,             /*!< @if Eng WAPI Personal.
                                             @else WAPI 个人级。 @endif */
    WS63_WIFI_SEC_TYPE_WAPI_CERT,            /*!< @if Eng WAPI Enterprise.
                                             @else WAPI 企业级。 @endif */
    WS63_WIFI_SEC_TYPE_WPA3_WPA2_MIX,        /*!< @if Eng WPA2-Enterprise and WPA3-Enterprise mixed.
                                             @else 企业级的WPA2和WPA3混合。 @endif */
    WS63_WIFI_SEC_TYPE_WEP_OPEN,             /*!< @if Eng Wired Equivalent Privacy OPEN system (WEP-OPEN).
                                             @else WEP-OPEN。 @endif */
    WS63_WIFI_SEC_TYPE_UNKNOWN               /*!< @if Eng Other certification types.
                                             @else 其它认证类型。 @endif */
} wifi_security_enum;
 ...
 */
typedef enum {
    WS63_STATIC_IP,  /*!< @if Eng Static IP address.
                     @else 静态IP地址。 @endif */
    WS63_DHCP,       /*!< @if Eng IP address dynamically assigned by DHCP.
                     @else 由DHCP动态分配的IP地址。 @endif */
    WS63_UNKNOWN     /*!< @if Eng Unknown IP address type.
 ...
 */
typedef enum {
    WS63_WIFI_BASIC_SCAN,        /*!< @if Eng A basic scan.
                                 @else 普通扫描 @endif */
    WS63_WIFI_CHANNEL_SCAN,      /*!< @if Eng A scan based on a specified chanel.
                                 @else 基于指定信道的扫描。 @endif */
    WS63_WIFI_SSID_SCAN,         /*!< @if Eng A scan based on a specified SSID.
                                 @else 基于指定SSID的扫描。 @endif */
    WS63_WIFI_SSID_PREFIX_SCAN,  /*!< @if Eng  A scan based on a specified Prefix SSID.
                                 @else 基于指定前缀SSID的扫描。 @endif */
    WS63_WIFI_BSSID_SCAN,        /*!< @if Eng  A scan based on a specified BSSID.
