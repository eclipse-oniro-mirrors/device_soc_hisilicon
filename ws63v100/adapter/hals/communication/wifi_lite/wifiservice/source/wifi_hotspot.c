...
#include "ws63_wifi_hotspot_config.h"
#include <securec.h>
...
WifiErrorCode EnableHotspot(void) {
    ...
    softap_config_stru hapdConf = {0};

    hapdConf.channel_num = HOTSPOT_DEFAULT_CHANNEL;
    if (g_wifiApConfig.channelNum) {
        hapdConf.channel_num = g_wifiApConfig.channelNum;
    }
    hapdConf.security_type = g_wifiApConfig.securityType;


    int cpyErr = memcpy_s(hapdConf.ssid, WIFI_MAX_SSID_LEN, g_wifiApConfig.ssid, WIFI_MAX_SSID_LEN);
    cpyErr += memcpy_s(hapdConf.pre_shared_key, WIFI_MAX_KEY_LEN, g_wifiApConfig.preSharedKey, WIFI_MAX_SSID_LEN);
    if (cpyErr != EOK) {
    ...
    int hiRet = wifi_softap_enable(&hapdConf);
    if (hiRet != 0) {
    ...
    cpyErr = memset_s(&hapdConf, sizeof(softap_config_stru), 0, sizeof(softap_config_stru));
    if (cpyErr != EOK) {
...
WifiErrorCode GetStationList(StationInfo *result, unsigned int *size) {
    ...
    wifi_sta_info_stru staList[WIFI_MAX_STA_NUM] = {0};
    unsigned int staNum = WIFI_MAX_STA_NUM;

    int hiRet = wifi_softap_get_sta_list(staList, &staNum);
    if (hiRet != 0) {
    ...
    for (unsigned int i = 0; i < staNum; i++) {
        cpyErr = memcpy_s(result[i].macAddress, WIFI_MAC_LEN, staList[i].mac_addr, WIFI_MAC_LEN);
        if (cpyErr != EOK) {
        ...
        int ret = GetPeerSTAWifiIp(staList[i].mac_addr, WIFI_MAC_LEN, &(result[i].ipAddress));
        if (ret != 0) {
...
//    }
//
//    uapi_wifi_add_usr_app_ie(WIFI_IFTYPE_AP,)
//    int ret = hi_wifi_add_usr_app_ie(WIFI_IFTYPE_AP, HI_WIFI_FRAME_TYPE_BEACON, HI_WIFI_USR_IE_TYPE_DEFAULT,
