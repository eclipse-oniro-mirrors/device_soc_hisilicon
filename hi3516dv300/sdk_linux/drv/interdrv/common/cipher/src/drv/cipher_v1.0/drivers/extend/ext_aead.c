...
hi_s32 ext_mbedtls_aead_setkey(hi_void *ctx, const hi_u8 *fkey, const hi_u8 *skey, hi_u32 *hisi_klen)
{
    ...
    if (memcpy_s(aead->key, SYMC_KEY_SIZE, fkey, klen) != EOK) {
        hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
        return HI_ERR_CIPHER_MEMCPY_S_FAILED;
    }
    aead->klen = klen;
...
hi_s32 ext_mbedtls_aead_get_tag(hi_void *ctx, hi_u32 tag[AEAD_TAG_SIZE_IN_WORD], hi_u32 *taglen)
{
    ...
    *taglen = aead->tlen;
    ...
    if (memcpy_s(tag, AEAD_TAG_SIZE, aead->tag, aead->tlen) != EOK) {
        hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
        return HI_ERR_CIPHER_MEMCPY_S_FAILED;
    }
...
hi_s32 ext_mbedtls_aead_ccm_crypto(hi_void *ctx, hi_u32 operation, symc_multi_pack *pack, hi_u32 last)
{
    ...
    (hi_void)memset_s(&mem, sizeof(ext_ccm_gcm_mem), 0, sizeof(ext_ccm_gcm_mem));
    (hi_void)memset_s(&ccm, sizeof(mbedtls_ccm_context), 0, sizeof(mbedtls_ccm_context));
    ...
    hi_log_debug("aead 0x%pK, klen len: %u\n", aead, aead->klen);
...
hi_s32 ext_mbedtls_aead_gcm_crypto(hi_void *ctx, hi_u32 operation, symc_multi_pack *pack, hi_u32 last)
{
    ...
    (hi_void)memset_s(&mem, sizeof(mem), 0, sizeof(mem));
