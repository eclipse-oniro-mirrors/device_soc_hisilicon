...
static hi_s32 kapi_hash_create(hi_u32 *id)
{
    ...
    *id = chn;
    ...
    hi_log_debug("kapi create soft chn %u\n", chn);
...
static hi_void kapi_hmac_cal_ipad_opad(kapi_hash_ctx *ctx)
{
    hi_u32 i;
...
static hi_s32 kapi_hmac_start(kapi_hash_ctx *ctx, const hi_u8 *key, hi_u32 keylen)
{
    hi_s32 ret;
    ...
    if ((memset_s(ctx->hmac_ipad, HASH_BLOCK_SIZE_128, 0x00, ctx->func->block_size) != EOK) ||
        ...
        hi_log_print_func_err(memset_s, HI_ERR_CIPHER_MEMSET_S_FAILED);
    ...
    if (keylen <= ctx->func->block_size) {
        ...
        if (ctx->cryp_ctx == HI_NULL) {
            hi_log_print_func_err(ctx->func->create, HI_ERR_CIPHER_BUSY);
            return HI_ERR_CIPHER_BUSY;
        }

        /* update key */
        crypto_chk_err_goto(ctx->func->update(ctx->cryp_ctx, key, keylen, HASH_CHUNCK_SRC_LOCAL));

        /* sum */
        crypto_chk_err_goto(ctx->func->finish(ctx->cryp_ctx, sum, sizeof(sum), &len));
    ...
    crypto_chk_err_goto(ctx->func->update(ctx->cryp_ctx, ctx->hmac_ipad, ctx->func->block_size, HASH_CHUNCK_SRC_LOCAL));
...
exit__:
    ctx->func->destroy(ctx->cryp_ctx);
    ctx->cryp_ctx = HI_NULL;
    (hi_void)memset_s(ctx->hmac_ipad, sizeof(ctx->hmac_ipad), 0, sizeof(ctx->hmac_ipad));
    (hi_void)memset_s(ctx->hmac_opad, sizeof(ctx->hmac_opad), 0, sizeof(ctx->hmac_opad));
    return ret;
}
...
{
    (hi_void)memset_s(ctx, sizeof(kapi_hash_ctx), 0, sizeof(kapi_hash_ctx));
    /* record owner */
    ...
    if ((ctx->func->create == HI_NULL) || (ctx->func->update == HI_NULL) ||
        (ctx->func->finish == HI_NULL) || (ctx->func->destroy == HI_NULL)) {
...
static hi_s32 kapi_hash_create_calc(kapi_hash_ctx *ctx,
    hash_mode mode, hi_u32 hmac, const hi_u8 *key, hi_u32 keylen)
{
    hi_s32 ret;
...
static hi_s32 kapi_hash_chk_type(hi_cipher_hash_type type)
{
    ...
    if (type == HI_CIPHER_HASH_TYPE_SHA1 || type == HI_CIPHER_HASH_TYPE_HMAC_SHA1) {
        hi_log_error("Invalid alg, unsupported sha1.\n");
    ...
    if (type == HI_CIPHER_HASH_TYPE_SHA224 || type == HI_CIPHER_HASH_TYPE_HMAC_SHA224) {
        hi_log_error("Invalid alg, unsupported sha224.\n");
        hi_log_print_err_code(HI_ERR_CIPHER_INVALID_PARA);
        return HI_ERR_CIPHER_INVALID_PARA;
    }
#endif
...
hi_s32 kapi_hash_start(hi_u32 *id, hi_cipher_hash_type type, const hi_u8 *key, hi_u32 keylen)
{
    ...
    *id = hi_handle_init(HI_ID_CIPHER, 0, soft_hash_id);
    kapi_hash_unlock();
    hi_log_func_exit();
    return HI_SUCCESS;
...
hi_s32 kapi_hash_update(hi_u32 id, const hi_u8 *input, hi_u32 length, hash_chunk_src src)
{
    hi_s32 ret;
    kapi_hash_ctx *ctx = HI_NULL;
    hi_u32 soft_hash_id = 0;
    ...
    hi_log_chk_param_return(ctx->cryp_ctx == HI_NULL);
...
hi_s32 kapi_hash_release(hi_void)
{
    ...
    for (i = 0; i < HASH_SOFT_CHANNEL_MAX; i++) {
        if (g_hash_ctx[i].open == HI_TRUE) {
            ...
            hi_log_info("hash release chn %u\n", i);
