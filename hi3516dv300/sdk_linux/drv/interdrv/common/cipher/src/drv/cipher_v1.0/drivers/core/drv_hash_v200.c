...
static hi_s32 drv_hash_get_err_code(hi_u32 chn_num)
{
    ...
    if (code & KLAD_KEY_USE_ERR) {
        hi_log_error("hash error: klad_key_use_err, chn %u !!!\n", chn_num);
    }
    if (code & ALG_LEN_ERR) {
        hi_log_error("hash error: alg_len_err, chn %u !!!\n", chn_num);
    }
    if (code & SMMU_PAGE_INVALID) {
        hi_log_error("hash error: smmu_page_invalid, chn %u !!!\n", chn_num);
    }
...
static CRYPTO_IRQRETURN_T hash_interrupt_isr(hi_s32 irq, hi_void *dev_id)
{
    hi_u32 mask, i;
    hash_hard_context *ctx = HI_NULL;
...
static hi_void drv_hash_unregister_interrupt(hi_void)
{
    ...
    /* free irq */
    hi_log_debug("hash free irq, num %u, name %s\n", int_num, name);
...
static hi_void hash_set_interrupt(hi_void)
{
    ...
    int_en.u32 = hash_read(HASH_INT_EN);

    if (int_valid == HI_FALSE) {
        /* The top interrupt switch only can be enable/disable by secure CPU. */
        int_en.bits.hash_int_en = 0;
        int_en.bits.hash_sec_int_en = 0;
        int_en.bits.hash_chn_oram_en &= ~HASH_HARD_CHANNEL_MASK;
    } else {
        /* The top interrupt switch only can be enable/disable by secure CPU. */
        int_en.bits.hash_int_en = 1;
        int_en.bits.hash_sec_int_en = 1;
        int_en.bits.hash_chn_oram_en |= HASH_HARD_CHANNEL_MASK;
    }

    hash_write(HASH_INT_EN, int_en.u32);
     ...
     */
    int_raw.u32 = symc_read(HASH_INT_RAW);
    int_raw.bits.hash_chn_oram_raw &= HASH_HARD_CHANNEL_MASK; /* clear valid channel */
    symc_write(HASH_INT_RAW, int_raw.u32);
...
/* set hash entry */
static hi_void hash_set_entry(hi_u32 chn, compat_addr dma_addr, const hi_void *cpu_addr)
{
    ...
    hi_log_info("chann_hash_in_node_cfg[0x%x]: \t0x%x, PHY: 0x%x, VIA %pK\n",
...
static hi_void drv_hash_enable_secure(hi_void)
{
    ...
    chn_cfg.u32 = hash_read(SEC_CHN_CFG);
    chn_cfg.bits.hash_sec_chn_cfg |= HASH_HARD_CHANNEL_MASK;
    hash_write(SEC_CHN_CFG, chn_cfg.u32);
    hi_log_info("SEC_CHN_CFG[0x%x]: 0x%x\n", SEC_CHN_CFG, chn_cfg.u32);
}

static hi_void hash_entry_init(crypto_mem mem)
{
    hi_u32 i;
...
static hi_s32 drv_hash_cfg_init(hi_void)
{
    ...
    if (ret != HI_SUCCESS) {
        ...
        ret_error = hash_mem_destroy(&g_hash_dma);
        if (ret_error != HI_SUCCESS) {
            hi_log_print_func_err(hash_mem_destroy, ret_error);
        }
        return ret;
    }
#endif
...
hi_s32 drv_hash_deinit(hi_void)
{
    ...
    ret = hash_mem_destroy(&g_hash_dma);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(hash_mem_destroy, ret);
        return ret;
    }
    module_disable(CRYPTO_MODULE_ID_HASH);
...
static hi_s32 drv_hash_query_raw_interrupt(hi_u32 chnnel_id)
{
    ...
    if (i >= CRYPTO_TIME_OUT) {
        ...
        hi_log_error("hash wait done timeout, chn=%u\n", chnnel_id);
...
static hi_void hash_addbuf(hi_u32 chn_num, compat_addr buf_phy, hi_u32 buf_size)
{
    ...
    hi_log_info("add digest in buf: id %u, addr 0x%x, len 0x%x\n", id, addr_l32(buf_phy), buf_size);
}

hi_s32 drv_hash_cfg(hi_u32 chn_num, hash_mode mode, const hi_u32 state[HASH_RESULT_MAX_SIZE_IN_WORD])
{
    hash_hard_context *ctx = HI_NULL;
...
hi_s32 drv_hash_start(hi_u32 chn_num, const crypto_mem *mem, hi_u32 length)
{
...
{
    (hi_void)memset_s(capacity, sizeof(hash_capacity), 0,  sizeof(hash_capacity));
