...
hi_void hal_cipher_start_klad(hi_u32 block_num)
{
    ...
    if (block_num == KLAD_HIGH_128BIT_BLOCK) {
...
hi_void hal_cipher_get_klad_data(hi_u32 *data_out, hi_u32 data_len_in_word)
{
    ...
    /* The length of klad output key is 16 bytes. */
    for (i = 0; i < data_len_in_word; i++) {
...
static hi_void hal_cipher_klad_init(hi_void)
{
    hi_u32 crg_value = 0;
    ...
    crg_value |= KLAD_CRG_CLOCK_BIT;   /* set the bit 1, clock opened */
    ...
    hi_cipher_ca_type root_key, hi_cipher_klad_target target, const hi_u8 *data_in, hi_u32 key_len)
{
    ...
    otp_id = root_key - HI_CIPHER_KEY_SRC_KLAD_1 + BOUND_VAL_1;

    ret = hal_cipher_klad_config(chn_id, otp_id, target, HI_TRUE);
    if (ret != HI_SUCCESS) {
    ...
    for (i = 0; i < key_len / AES_BLOCK_SIZE; i++) {
        if (memcpy_s(key, sizeof(key), data_in + i * AES_BLOCK_SIZE, AES_BLOCK_SIZE) != EOK) {
        ...
        if (ret != HI_SUCCESS) {
            ...
            (hi_void)memset_s(key, sizeof(key), 0, sizeof(key));
            return ret;
        }
    }
...
hi_s32 drv_cipher_klad_encrypt_key(hi_cipher_ca_type root_key,
    hi_cipher_klad_target target, hi_u8 *clean_key, hi_u8 *encrypt_key, hi_u32 key_len)
{
    hi_s32 ret;
    hi_u32 otp_id;
    ...
    ret = hal_cipher_klad_config(0, otp_id, HI_CIPHER_KLAD_TARGET_AES, HI_FALSE);
    if (ret != HI_SUCCESS) {
        hi_log_error("Error: cipher klad config failed!\n");
        return ret;
    }
