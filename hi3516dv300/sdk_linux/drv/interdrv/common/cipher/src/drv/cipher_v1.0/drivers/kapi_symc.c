...
typedef struct {
    ...
    hi_u32 config : 1;                  /* already config or not */
    symc_func *func;
    void *cryp_ctx;                     /* Context of cryp instance */
    CRYPTO_OWNER owner;                 /* user ID */
    hi_cipher_ctrl  ctrl;               /* control information */
} kapi_symc_ctx;
...
hi_s32 kapi_symc_init(void)
{
    ...
    (hi_void)memset_s(g_kapi_ctx, sizeof(g_kapi_ctx), 0, sizeof(g_kapi_ctx));
...
hi_s32 kapi_symc_release(void)
{
    ...
    for (i = 0; i < CRYPTO_HARD_CHANNEL_MAX; i++) {
        ...
        if (ctx->open == HI_TRUE) {
            ...
            hi_log_info("symc release chn %u\n", chn);
...
hi_s32 kapi_symc_create(hi_u32 *id)
{
    ...
    (hi_void)memset_s(ctx, sizeof(kapi_symc_ctx), 0, sizeof(kapi_symc_ctx));
    ...
    *id = hi_handle_init(HI_ID_CIPHER, 0, chn);
    ...
    hi_log_info("kapi_symc_create()- chn %u\n", chn);
...
static hi_s32 kapi_symc_chk_des_3des_param(hi_cipher_alg alg, hi_cipher_work_mode mode, hi_u32 width)
{
    ...
    if (alg == HI_CIPHER_ALG_DES) {
        hi_log_error("Invalid alg, unsupported des.\n");
    ...
    if (alg == HI_CIPHER_ALG_3DES) {
        hi_log_error("Invalid alg, unsupported 3des.\n");
        hi_log_print_err_code(HI_ERR_CIPHER_INVALID_PARA);
        return HI_ERR_CIPHER_INVALID_PARA;
    }
#endif
...
static hi_s32 kapi_symc_chk_aes_param(hi_cipher_alg alg, hi_cipher_work_mode mode, hi_u32 width)
{
    ...
    if ((mode == HI_CIPHER_WORK_MODE_CFB) && (width != SYMC_DAT_WIDTH_1) && (width != SYMC_DAT_WIDTH_8) &&
        ...
        hi_log_error("Invalid alg %d mode %d and width %u\n", alg, mode, width);
        hi_log_print_err_code(HI_ERR_CIPHER_INVALID_PARAM);
        return HI_ERR_CIPHER_INVALID_PARAM;
    }
...
static hi_s32 kapi_symc_chk_sm1_param(hi_cipher_alg alg, hi_cipher_work_mode mode, hi_u32 width, hi_u32 round)
{
    ...
    if ((alg == HI_CIPHER_ALG_SM1) && (round >= HI_CIPHER_SM1_ROUND_BUTT)) {
        hi_log_error("Invalid alg %d and Sm1Round %u\n", alg, round);
        hi_log_print_err_code(HI_ERR_CIPHER_INVALID_PARAM);
        return HI_ERR_CIPHER_INVALID_PARAM;
    }
...
static hi_s32 kapi_symc_match_width(hi_cipher_work_mode work_mode, hi_cipher_bit_width bit_width, symc_width *width)
{
    ...
    if ((work_mode == HI_CIPHER_WORK_MODE_CFB) || (work_mode == HI_CIPHER_WORK_MODE_OFB)) {
        ...
        if (bit_width == HI_CIPHER_BIT_WIDTH_128BIT) {
            *width = SYMC_DAT_WIDTH_128;
        } else if (bit_width == HI_CIPHER_BIT_WIDTH_64BIT) {
            *width = SYMC_DAT_WIDTH_64;
        } else {
            hi_log_error("Invalid width: 0x%x, mode 0x%x\n", bit_width, work_mode);
            hi_log_print_err_code(HI_ERR_CIPHER_INVALID_PARAM);
...
static hi_s32 kapi_symc_chk_param(hi_u32 hard_key, const symc_cfg_t *cfg, symc_width *width)
{
    hi_s32 ret;
...
static hi_s32 kapi_symc_chk_cfg(const symc_cfg_t *cfg, symc_set_cfg_param *set_cfg)
{
    hi_s32 ret;
    kapi_symc_ctx *ctx = HI_NULL;
...
static hi_void kapi_symc_set_alg_mode(const kapi_symc_ctx *ctx, const symc_set_cfg_param *set_cfg)
{
    /* set mode and alg */
    if (ctx->func->setmode) {
...
static hi_s32 kapi_symc_set_key(const symc_cfg_t *cfg,
    const kapi_symc_ctx *ctx, symc_set_cfg_param *set_cfg)
{
    hi_s32 ret;
...
static hi_s32 kapi_symc_cpy_key_iv(const symc_cfg_t *cfg,
    kapi_symc_ctx *ctx, const symc_set_cfg_param *set_cfg)
{
    if (cfg->iv != HI_NULL) {
        ...
        if (memcpy_s(ctx->ctrl.iv, AES_IV_SIZE, cfg->iv, cfg->ivlen) != EOK) {
    ...
    if (cfg->fkey != HI_NULL) {
        ...
        if (memcpy_s(ctx->ctrl.key, AES_KEY_256BIT, cfg->fkey, set_cfg->klen) != EOK) {
            hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
            return HI_ERR_CIPHER_MEMCPY_S_FAILED;
        }
    }
...
static hi_s32 kapi_symc_cfg_set_param(const symc_cfg_t *cfg, kapi_symc_ctx *ctx, symc_set_cfg_param *set_cfg)
{
    hi_s32 ret;
    ...
    if (ctx->func->create) {
        ...
        if (ctx->cryp_ctx == HI_NULL) {
            hi_log_error("attach context buffer to soft_id %u failed\n", set_cfg->soft_id);
    ...
    crypto_chk_err_goto(kapi_symc_set_key(cfg, ctx, set_cfg));

    /* Set IV */
    if (ctx->func->setiv) {
        crypto_chk_err_goto(ctx->func->setiv(ctx->cryp_ctx, cfg->iv, cfg->ivlen, cfg->iv_usage));
    }

    /* set sm1 round num */
    if (ctx->func->setround) {
        crypto_chk_err_goto(ctx->func->setround(ctx->cryp_ctx, cfg->sm1_round_num));
    }

    /* Set AAD */
    if (ctx->func->setadd) {
        crypto_chk_err_goto(cipher_check_mmz_phy_addr((hi_phys_addr_t)addr_u64(cfg->aad), cfg->alen));
        hi_log_info("set add, phy 0x%x, alen %u, tlen %u\n", addr_l32(cfg->aad), cfg->alen, cfg->tlen);
        crypto_chk_err_goto(kapi_symc_check_ccm_gcm_taglen(cfg->alg, cfg->mode, cfg->tlen));
        crypto_chk_err_goto(ctx->func->setadd(ctx->cryp_ctx, cfg->aad, cfg->alen, cfg->tlen));
    }

    /* save ctrl */
    (hi_void)memset_s(&ctx->ctrl, sizeof(hi_cipher_ctrl), 0, sizeof(hi_cipher_ctrl));
    ...
    crypto_chk_err_goto(kapi_symc_cpy_key_iv(cfg, ctx, set_cfg));
    ctx->config = HI_TRUE;
    return HI_SUCCESS;
exit__:
    return ret;
...
hi_s32 kapi_symc_cfg(const symc_cfg_t *cfg)
{
    hi_s32 ret;
    kapi_symc_ctx *ctx = HI_NULL;
    ...
    (hi_void)memset_s(&set_cfg, sizeof(set_cfg), 0, sizeof(set_cfg));
...
hi_s32 kapi_symc_get_cfg(hi_u32 id, hi_cipher_ctrl *ctrl)
{
    ...
    if (ctx->func->getiv != HI_NULL) {
        ctx->func->getiv(ctx->cryp_ctx, (hi_u8 *)ctrl->iv, &iv_len);
    }

    if (memcpy_s(ctrl, sizeof(hi_cipher_ctrl), &ctx->ctrl, sizeof(hi_cipher_ctrl)) != EOK) {
        kapi_symc_unlock();
        hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
        return HI_ERR_CIPHER_MEMCPY_S_FAILED;
    }
    (hi_void)memset_s(ctrl->key, sizeof(ctrl->key), 0, sizeof(ctrl->key));
...
hi_s32 kapi_symc_crypto(symc_encrypt_t *crypt)
{
    ...
    (hi_void)memset_s(&pack, sizeof(pack), 0, sizeof(pack));
    pack.in = &crypt->in;
    pack.out = &crypt->out;
    pack.len = &crypt->len;
    pack.usage = &usage;
...
hi_s32 kapi_symc_crypto_via(symc_encrypt_t *crypt, hi_u32 is_from_user)
{
    ...
    (hi_void)memset_s(&local_crypt, sizeof(local_crypt), 0, sizeof(local_crypt));
    ...
    if (is_from_user == HI_TRUE) {
        crypto_chk_err_goto(crypto_copy_from_user(mem.dma_virt, crypt->len, addr_via(crypt->in), crypt->len));
    } else {
        if (memcpy_s(mem.dma_virt, crypt->len, addr_via(crypt->in), crypt->len) != EOK) {
            hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
    ...
    crypto_chk_err_goto(kapi_symc_crypto(&local_crypt));

    if (is_from_user == HI_TRUE) {
        crypto_chk_err_goto(crypto_copy_to_user(addr_via(crypt->out), crypt->len, mem.dma_virt, crypt->len));
    } else {
        if (memcpy_s(addr_via(crypt->out), crypt->len, mem.dma_virt, crypt->len) != EOK) {
            hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
            ret = HI_ERR_CIPHER_MEMCPY_S_FAILED;
            goto exit__;
        }
...
exit__:
    ...
    if (ret_exit != HI_SUCCESS) {
        hi_log_print_func_err(crypto_mem_destroy, ret_exit);
        return ret_exit;
    }
...
static hi_s32 kapi_symc_chk_multi_pack(hi_cipher_data *tmp, const hi_cipher_data *pack)
{
    hi_s32 ret;

    /* copy node list from user space to kernel. */
    ret = crypto_copy_from_user(tmp, sizeof(hi_cipher_data), pack, sizeof(hi_cipher_data));
    if (ret != HI_SUCCESS) {
...
static hi_void kapi_symc_multi_pack_set_mem(symc_multi_pack *pack, const hi_u8 *buf, hi_u32 size)
{
    const hi_u8 *tmp = HI_NULL;
...
static hi_s32 kapi_symc_crypto_multi_start(const kapi_symc_ctx *ctx,
    const hi_cipher_data *pkg, hi_u32 pkg_num, hi_u32 operation, hi_u32 wait)
{
    hi_s32 ret;
    hi_void *buf = HI_NULL;
    ...
    hi_log_chk_param_return((ctx == HI_NULL) || (ctx->cryp_ctx == HI_NULL) ||
        (ctx->func == HI_NULL) || (ctx->func->crypto == HI_NULL));
    ...
    for (i = 0; i < pkg_num; i++) {
        ...
        hi_log_debug("pkg %u, in 0x%x, out 0x%x, length 0x%x, usage 0x%x\n", i, addr_l32(pack.in[i]),
...
hi_s32 kapi_symc_crypto_multi(hi_u32 id, const hi_cipher_data *pkg, hi_u32 pkg_num, hi_u32 operation, hi_u32 last)
{
    hi_s32 ret;
    kapi_symc_ctx *ctx = HI_NULL;
    hi_u32 soft_id;
...
hi_s32 kapi_symc_klad_encrypt_key(hi_u32 keysel,
    hi_u32 target, hi_u8 *clear, hi_u8 *encrypt, hi_u32 key_len)
{
    hi_s32 ret;
    ...
    ret = klad_encrypt_key(keysel, target, clear, encrypt, key_len);
    if (ret != HI_SUCCESS) {
