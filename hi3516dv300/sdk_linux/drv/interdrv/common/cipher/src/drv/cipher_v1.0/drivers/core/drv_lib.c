 ...
 * the table of base info of module, such as addr/reset/clk/ver.
 */
...
static hi_s32 module_addr_map_reg(hi_void)
{
    ...
    for (i = 0; i < CRYPTO_MODULE_ID_CNT; i++) {
        ...
        if (table->crg_valid) {
            ...
            hi_log_info("crg phy 0x%x, via 0x%pK\n", table->crg_addr_phy, table->crg_addr_via);
        }

        /* io remap module register. */
        if (table->phy_valid) {
            table->reg_addr_via = crypto_ioremap_nocache(table->reg_addr_phy, table->reg_addr_size);
            ...
            hi_log_info("reg phy 0x%x, via 0x%pK, size 0x%x\n", table->reg_addr_phy,
...
hi_s32 module_addr_unmap(hi_void)
{
    ...
    for (i = 0; i < CRYPTO_MODULE_ID_CNT; i++) {
        ...
        if (table->crg_valid && table->crg_addr_via) {
            hi_log_info("crg via addr 0x%pK\n", table->crg_addr_via);
        ...
        if (table->phy_valid && table->reg_addr_via) {
            hi_log_info("reg via addr 0x%pK\n", table->reg_addr_via);
...
static hi_void module_clock(module_id id, hi_u32 open)
{
    ...
    if (id >= CRYPTO_MODULE_ID_CNT) {
        hi_log_error("invalid module id %d\n", id);
        return;
    }
    ...
    if (table->clk_valid) {
        ...
        hi_log_info("%s clock, open %u, bit %u, phy 0x%x, via 0x%pK\n",
...
static hi_void module_reset(module_id id, hi_u32 enable)
{
    ...
    if (table->reset_valid) {
        ...
        if (table->crg_addr_via == HI_NULL) {
            hi_log_error("%s reset failed, crg addr is null\n", table->name);
            return;
        }
        ...
        hi_log_info("%s reset, enable %u, bit %u, phy 0x%x, via 0x%pK\n",
        ...
        if (table->ver_valid && table->reg_addr_via) {
            ...
            if (val != expect) {
                hi_log_error("%s reset failed, version reg should be 0x%x but 0x%x\n", table->name, expect, val);
                return;
            }
 ...
 */
hi_void module_reg_write(module_id mod_id, hi_u32 offset, hi_u32 val)
{
    hi_void *addr = HI_NULL;
    hi_s32 ret;
    ...
    if (mod_id >= CRYPTO_MODULE_ID_CNT) {
        hi_log_error("error, invalid mod_id %d\n", mod_id);
        return;
    }
    ...
    /* tee may be read trng before cipher module init. */
    if (g_hard_info_table[mod_id].reg_addr_via == HI_NULL) {
        ret = module_addr_map();
        if (ret != HI_SUCCESS) {
            hi_log_print_func_err(module_addr_map, ret);
            return;
    ...
    /* get the absolute address of reg. */
    addr = (hi_u8 *)g_hard_info_table[mod_id].reg_addr_via + offset;
    crypto_write(addr, val);
...
hi_s32 crypto_channel_init(channel_context *ctx, hi_u32 num, hi_u32 ctx_size)
{
    ...
    (hi_void)memset_s(ctx, size, 0, size);
    ...
    if (ctx_size > 0) {
        ...
        (hi_void)memset_s(buf, ctx_size * num, 0, ctx_size * num);
 ...
 */
hi_void *crypto_channel_get_context(const channel_context *ctx, hi_u32 num, hi_u32 id)
{
    if (ctx == HI_NULL) {
...
hi_void *crypto_calloc(size_t n, size_t size)
{
    ...
    if (ptr != HI_NULL) {
        (hi_void)memset_s(ptr, n * size, 0, n * size);
    }
    return ptr;
}
...
void crypto_print_hex(const hi_char *name, hi_u8 *str, hi_u32 len)
{
    ...
    HI_PRINT("[%s]:\n", (name));
    for (_i = 0; _i < (len); _i++) {
        if ((_i % PRINT_HEX_BLOCK_LEN == 0) && (_i != 0)) {
            HI_PRINT("\n");
        }
