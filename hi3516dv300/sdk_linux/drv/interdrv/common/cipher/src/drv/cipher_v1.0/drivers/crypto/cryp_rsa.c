...
#ifdef SOFT_RSA_PADDING_SUPPORT
static hi_u32 g_rsa_key_ca_type = HI_CIPHER_KEY_SRC_USER;

/* API Code for cryp rsa */
static hi_void mbedtls_mpi_print(const mbedtls_mpi *x, const char *name)
{
    ...
    if (n > RSA_KEY_WIDTH_4096) {
        hi_log_error("length overflow!\n");
        return;
    }
...
{
    if (module_len > RSA_KEY_LEN_4096) {
        hi_log_error("error, invalid key len %lu\n", module_len);
        hi_log_print_err_code(HI_ERR_CIPHER_INVALID_PARAM);
        return HI_ERR_CIPHER_INVALID_PARAM;
#ifdef CHIP_RSA1024_SUPPORT
    } else if (module_len <= RSA_KEY_LEN_1024) {
...
#ifdef CHIP_RSA3072_SUPPORT
    } else if (module_len <= RSA_KEY_LEN_3072) {
...
static hi_s32 cryp_check_data(const hi_u8 *n, const hi_u8 *e, const hi_u8 *mc, hi_u32 len)
{
    hi_u32 i;
                                 ...
                                 const mbedtls_mpi *e, const mbedtls_mpi *n)
{
    ...
    /* malloc buf to store n || k(e or d) || in || out */
    ...
    crypto_chk_err_goto(mbedtls_mpi_mod_mpi(&tmp_a, a, n));

    /* read A, E, N */
    crypto_chk_err_goto(mbedtls_mpi_write_binary(&tmp_a, in, keylen));
    crypto_chk_err_goto(mbedtls_mpi_write_binary(e, k, keylen));
    crypto_chk_err_goto(mbedtls_mpi_write_binary(n, tmp_n, keylen));

    /* key and data valid ? */
    crypto_chk_err_goto(cryp_check_data(tmp_n, k, in, keylen));
    ...
    mbedtls_mpi *rr)
{
    ...
    if (elen <= RSA_KEY_LEN_4096) {
         ...
         */
        ret = cryp_ifep_rsa_exp_mod(g_rsa_key_ca_type, x, a, e, n);
    } else {
        if (g_rsa_key_ca_type != HI_CIPHER_KEY_SRC_USER) {
            hi_log_error("software rsa nonsupport klad key\n");
...
{
    hi_s32 ret;
    hi_u32 i, randnum, left_size;

    crypto_unused(param);

    hi_log_chk_param_return(rand == HI_NULL);

    for (i = 0; i < (hi_u32)size; i += WORD_WIDTH) {
        ret = cryp_trng_get_random(&randnum, CRYP_TRNG_TIMEOUT);
        if (ret != HI_SUCCESS) {
            hi_log_print_func_err(cryp_trng_get_random, ret);
            return ret;
        }
...
static hi_s32 cryp_rsa_init_key(const cryp_rsa_key *key, hi_u32 *mode, mbedtls_rsa_context *rsa)
{
    hi_s32 ret;
    ...
    crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->N, key->n, key->klen));
    rsa->len = ((mbedtls_mpi_bitlen(&rsa->N) + 7) >> SHIFT_3BITS); /* 7 make sure to round up */
    ...
    if (key->public) {
        crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->E, (hi_u8 *)&key->e, WORD_WIDTH));
        *mode = MBEDTLS_RSA_PUBLIC;
    } else {
        if (key->d != HI_NULL) { /* Non CRT */
            crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->D, key->d, key->klen));
            *mode = MBEDTLS_RSA_PRIVATE;
        } else { /* CRT */
            crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->P, key->p, key->klen / MUL_VAL_2));
            crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->Q, key->q, key->klen / MUL_VAL_2));
            crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->DP, key->dp, key->klen / MUL_VAL_2));
            crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->DQ, key->dq, key->klen / MUL_VAL_2));
            crypto_chk_err_goto(mbedtls_mpi_read_binary(&rsa->QP, key->qp, key->klen / MUL_VAL_2));
            *mode = MBEDTLS_RSA_PRIVATE;
        }
    }
...
{
    switch (bt) {
        case RSA_BLOCK_TYPE_0:
            /* For block type 00, the octets shall have value 0x00 */
            (hi_void)memset_s(buf, buf_len, 0x00, buf_len);
            break;
        case RSA_BLOCK_TYPE_1:
            /* for block type 01, they shall have value 0xFF */
            (hi_void)memset_s(buf, buf_len, RSA_PADDING_VAL_FF, buf_len);
            break;
        case RSA_BLOCK_TYPE_2: {
            hi_u32 i;
...
static hi_s32 rsa_padding_add_pkcs1_type(mbedtls_rsa_context *rsa, rsa_padding_pack *pad)
{
    ...
    *(peb++) = 0x00;
    ...
    /* input data */
    if (memcpy_s(peb, pad->klen - (peb - pad->out), pad->in, pad->in_len) != EOK) {
...
static hi_s32 rsa_padding_check_pkcs1_type(mbedtls_rsa_context *rsa, rsa_padding_pack *pad)
{
    ...
    *(pad->out_len) = pad->klen - idx;
    if (memcpy_s(pad->out, pad->klen, &peb[idx], *(pad->out_len)) != EOK) {
        hi_log_print_func_err(memcpy_s, HI_ERR_CIPHER_MEMCPY_S_FAILED);
        return HI_ERR_CIPHER_MEMCPY_S_FAILED;
    }
...
static hi_s32 rsa_no_padding(mbedtls_rsa_context *rsa, rsa_padding_pack *no_pad)
{
    ...
    hi_log_chk_param_return(no_pad->in_len != no_pad->klen);

    ret = ext_rsa_calc(rsa, no_pad->mode, no_pad->in, no_pad->out, no_pad->in_len);
    if (ret != HI_SUCCESS) {
        hi_log_print_func_err(ext_rsa_calc, ret);
        return ret;
    }
...
hi_s32 cryp_rsa_encrypt(const cryp_rsa_key *key, cryp_rsa_crypt_data *rsa_crypt)
{
    ...
    (hi_void)memset_s(&pad, sizeof(pad), 0, sizeof(pad));
...
hi_s32 cryp_rsa_sign_hash(const cryp_rsa_key *key, cryp_rsa_sign_data *rsa_sign)
{
    ...
    hi_log_chk_param_return(rsa_sign->in_len < (hi_u32)hashlen);
...
hi_s32 cryp_rsa_verify_hash(const cryp_rsa_key *key, const cryp_rsa_sign_data *rsa_verify)
{
    ...
    hi_log_chk_param_return(rsa_verify->in_len < (hi_u32)hashlen);
...
hi_s32 cryp_rsa_bn_exp_mod(cryp_rsa_exp_mod *exp_mod)
{
    ...
    switch (exp_mod->length) {
        ...
        case RSA_KEY_LEN_4096:
            ...
            break;
        default:
            hi_log_error("Invalid key length %d\n", exp_mod->length);
            return HI_ERR_CIPHER_INVALID_PARAM;
    }

    kapi_rsa_lock_err_return();

    ret = drv_ifep_rsa_exp_mod(HI_CIPHER_KEY_SRC_USER,
        exp_mod->n, exp_mod->k, exp_mod->in, exp_mod->out, width);
    if (ret != HI_SUCCESS) {
        kapi_rsa_unlock();
        hi_log_print_func_err(drv_ifep_rsa_exp_mod, ret);
        return ret;
    }
