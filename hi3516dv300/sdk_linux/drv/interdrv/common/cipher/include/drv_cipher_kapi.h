...
#ifndef SM2_LEN_IN_WORD
#define SM2_LEN_IN_WORD                 8
...
#define CRYPTO_CMD_BN_EXP_MOD          crypto_iowr(0x10, sizeof(klad_key_t))
...
typedef union {
    ...
    hi_phys_addr_t phy;       /* physical address */
...
typedef struct {
    ...
    hi_u8 fkey[SYMC_KEY_SIZE];           /* first  key buffer, default */
...
typedef struct {
    ...
    hi_u32 pack_num;        /* Number of package information */
...
typedef struct {
    compat_addr n;      /* RSA input N */
    compat_addr k;      /* RSA input E/D */
    compat_addr in;     /* RSA input data */
    compat_addr out;    /* RSA output data */
    hi_u16 length;      /* RSA N/E/D, input/output data length */
} rsa_bn_exp_mod_t;

typedef struct {
    hi_u8 *n;           /* RSA input N */
    hi_u8 *k;           /* RSA input E/D */
    hi_u8 *in;          /* RSA input data */
    hi_u8 *out;         /* RSA output data */
    hi_u16 length;      /* RSA N/E/D, input/output data length */
} cryp_rsa_exp_mod;
 ...
 *
 * param[in] id     The channel number.
 * param pack       Buffer of package information
 * param pack_num   Number of package information
 * param operation  decrypt or encrypt
 * param last       last or not
 *
 * return           0 if successful
 */
hi_s32 kapi_symc_crypto_multi(hi_u32 id, const hi_cipher_data *pack, hi_u32 pack_num, hi_u32 operation, hi_u32 last);
