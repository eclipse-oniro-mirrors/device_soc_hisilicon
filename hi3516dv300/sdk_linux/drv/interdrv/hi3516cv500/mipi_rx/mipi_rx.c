...
{
    (void)memset_s(mipi_rx_hal_get_lvds_err_int_cnt(devno), sizeof(lvds_err_int_cnt_t), 0, sizeof(lvds_err_int_cnt_t));
    (void)memset_s(mipi_rx_hal_get_align_err_int_cnt(devno), sizeof(align_err_int_cnt_t), 0,
        sizeof(align_err_int_cnt_t));
...
{
    /* interrupt mask */
    (void)memset_s(mipi_rx_hal_get_mipi_err_int(devno), sizeof(mipi_err_int_cnt_t), 0, sizeof(mipi_err_int_cnt_t));
    (void)memset_s(mipi_rx_hal_get_align_err_int_cnt(devno), sizeof(align_err_int_cnt_t), 0,
        sizeof(align_err_int_cnt_t));
...
int mipi_set_ext_data_type(ext_data_type_t* data_type)
{
    ...
    for (i = 0; i < data_type->num; i++) {
        ...
        if (data_type->ext_data_bit_width[i] % 2 != 0) { /* 2:even check */
...
static int mipi_set_hs_mode(lane_divide_mode_t lane_divide_mode)
{
    ...
    (void)memset_s(g_mipirx_dev_ctx.dev_valid,
                   sizeof(g_mipirx_dev_ctx.dev_valid),
                   0,
                   sizeof(g_mipirx_dev_ctx.dev_valid));
...
static long mipi_rx_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    hi_mipi_rx_unused(private_data);
    switch (cmd) {
        case HI_MIPI_SET_DEV_ATTR: {
            ...
            if (ret < 0) {
                ...
                err = memcpy_s(&g_mipirx_dev_ctx.combo_dev_attr[devno],
                               sizeof(combo_dev_attr_t),
                               pstcombo_dev_attr,
                               sizeof(combo_dev_attr_t));
                if (err != EOK) {
                    g_mipirx_dev_ctx.dev_cfged[devno] = FALSE;
                    ret = HI_FAILURE;
                }
                osal_spin_unlock(&g_mipirx_ctx_lock);
            }
        ...
        case HI_MIPI_RESET_SENSOR: {
            ...
            err = memcpy_s(&sns_reset_source, sizeof(sns_rst_source_t), argp, sizeof(sns_rst_source_t));
            if (err != EOK) {
                ret = HI_FAILURE;
            } else {
        ...
        case HI_MIPI_RESET_MIPI: {
            ...
            err = memcpy_s(&combo_dev, sizeof(combo_dev_t), argp, sizeof(combo_dev_t));
            if (err != EOK) {
                ret = HI_FAILURE;
            } else {
        ...
        case HI_MIPI_SET_HS_MODE: {
            ...
            err = memcpy_s(&lane_divide_mode, sizeof(lane_divide_mode_t), argp, sizeof(lane_divide_mode_t));
            if (err != EOK) {
                ret = HI_FAILURE;
            } else {
        ...
        case HI_MIPI_ENABLE_SENSOR_CLOCK: {
            ...
            err = memcpy_s(&sns_clk_source, sizeof(sns_clk_source_t), argp, sizeof(sns_clk_source_t));
            if (err != EOK) {
                ret = HI_FAILURE;
            } else {
...
static void proc_show_phy_cil_int_err_cnt(osal_proc_entry_t *s)
{
    ...
    for (phy_id = 0; phy_id < MIPI_RX_MAX_PHY_NUM; phy_id++) {
        osal_seq_printf(s, "%8d%11d%10d%12d%12d%12d%12d%9d%8d%10d%10d%10d%10d\n",
            phy_id,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->clk1_fsm_timeout_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->clk0_fsm_timeout_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d0_fsm_timeout_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d1_fsm_timeout_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d2_fsm_timeout_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d3_fsm_timeout_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->clk1_fsm_escape_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->clk0_fsm_escape_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d0_fsm_escape_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d1_fsm_escape_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d2_fsm_escape_err_cnt,
            mipi_rx_hal_get_phy_err_int_cnt(phy_id)->d3_fsm_escape_err_cnt);
    }
}
...
static void proc_show_mipirx_crc_err(osal_proc_entry_t *s, const combo_dev_t devno_array[], int mipi_cnt)
{
    ...
    for (devno_idx = 0; devno_idx < mipi_cnt; devno_idx++) {
        ...
        osal_seq_printf(s, "%8d%6d%8d%8d%8d%8d%14d%14d%14d%14d\n",
            devno,
            mipi_rx_hal_get_mipi_err_int(devno)->err_ecc_double_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc0_err_crc_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc1_err_crc_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc2_err_crc_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc3_err_crc_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc0_err_ecc_corrected_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc1_err_ecc_corrected_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc2_err_ecc_corrected_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->vc3_err_ecc_corrected_cnt);
    }
}
...
static void proc_show_mipirx_vc_err(osal_proc_entry_t *s, const combo_dev_t devno_array[], int mipi_cnt)
{
    ...
    for (devno_idx = 0; devno_idx < mipi_cnt; devno_idx++) {
        ...
        osal_seq_printf(s, "%8d%7d%7d%7d%7d%11d%11d%11d%11d\n",
            devno,
            mipi_rx_hal_get_mipi_err_int(devno)->err_id_vc0_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_id_vc1_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_id_vc2_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_id_vc3_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_frame_data_vc0_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_frame_data_vc1_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_frame_data_vc2_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_frame_data_vc3_cnt);
    }
    ...
    for (devno_idx = 0; devno_idx < mipi_cnt; devno_idx++) {
        ...
        osal_seq_printf(s, "%8d%11d%11d%11d%11d%12d%12d%12d%12d\n",
            devno,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_seq_vc0_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_seq_vc1_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_seq_vc2_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_seq_vc3_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_bndry_match_vc0_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_bndry_match_vc1_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_bndry_match_vc2_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->err_f_bndry_match_vc3_cnt);
    }
}
...
static void proc_show_mipirx_fifo_err(osal_proc_entry_t *s, const combo_dev_t devno_array[], int mipi_cnt)
{
    ...
    for (devno_idx = 0; devno_idx < mipi_cnt; devno_idx++) {
        ...
        osal_seq_printf(s, "%8d%15d%14d%14d%15d\n",
            devno,
            mipi_rx_hal_get_mipi_err_int(devno)->data_fifo_rderr_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->cmd_fifo_rderr_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->data_fifo_wrerr_cnt,
            mipi_rx_hal_get_mipi_err_int(devno)->cmd_fifo_wrerr_cnt);
    }
}
...
static void proc_show_lvds_int_err(osal_proc_entry_t *s, const combo_dev_t devno_array[], int lvds_cnt)
{
    ...
    for (devno_idx = 0; devno_idx < lvds_cnt; devno_idx++) {
        ...
        osal_seq_printf(s, "%8d%10d%10d%8d%9d%12d%12d\n",
            devno,
            mipi_rx_hal_get_lvds_err_int_cnt(devno)->cmd_rd_err_cnt,
            mipi_rx_hal_get_lvds_err_int_cnt(devno)->cmd_wr_err_cnt,
            mipi_rx_hal_get_lvds_err_int_cnt(devno)->pop_err_cnt,
            mipi_rx_hal_get_lvds_err_int_cnt(devno)->lvds_state_err_cnt,
            mipi_rx_hal_get_lvds_err_int_cnt(devno)->link0_rd_err_cnt,
            mipi_rx_hal_get_lvds_err_int_cnt(devno)->link0_wr_err_cnt);
    }
}
...
static void proc_show_align_int_err(osal_proc_entry_t *s)
{
    ...
    for (devno = 0; devno < MIPI_RX_MAX_DEV_NUM; devno++) {
        ...
        osal_seq_printf(s, "%8d%14d%10d%10d%10d%10d\n",
            devno,
            mipi_rx_hal_get_align_err_int_cnt(devno)->fifo_full_err_cnt,
            mipi_rx_hal_get_align_err_int_cnt(devno)->lane0_align_err_cnt,
            mipi_rx_hal_get_align_err_int_cnt(devno)->lane1_align_err_cnt,
            mipi_rx_hal_get_align_err_int_cnt(devno)->lane2_align_err_cnt,
            mipi_rx_hal_get_align_err_int_cnt(devno)->lane3_align_err_cnt);
    }
}
...
static void mipi_rx_exit(void)
{
    ...
    osal_spin_lock_destroy(&g_mipirx_ctx_lock);
    osal_mutex_destroy(&g_mipirx_mutex);
}
