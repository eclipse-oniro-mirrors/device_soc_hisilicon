...
typedef struct {
    hi_u8       image_description[DCF_DESCRIPTION_LENGTH];        /* describes image */
    hi_u8       make[DCF_DESCRIPTION_LENGTH];          /* shows manufacturer of digital cameras */
    hi_u8       model[DCF_DESCRIPTION_LENGTH];         /* shows model number of digital cameras */
    hi_u8       software[DCF_DESCRIPTION_LENGTH];      /* shows firmware (internal software of digital cameras)
                                                        version number */

    hi_u8       light_source;        /* light source, actually this means white balance setting. '0' means unknown,
                                     '1' daylight, '2' fluorescent, '3' tungsten, '10' flash, '17' standard light A,
                                     '18' standard light B, '19' standard lightC, '20' D55, '21' D65,
                                     '22' D75, '255' other */
    hi_u32      focal_length;         /* focal length of lens used to take image. unit is millimeter */
    hi_u8       scene_type;           /* indicates the type of scene. value '0x01' means that the image was directly
                                        photographed. */
    hi_u8       custom_rendered;       /* indicates the use of special processing on image data, such as rendering
    ...
    hi_u8       scene_capture_type;       /* indicates the type of scene that was shot. 0 = standard, 1 = landscape,
                                                2 = portrait,3 = night scene. */
    hi_u8       gain_control;           /* indicates the degree of overall image gain adjustment. 0 = none,
                                        1 = low gain up,2 = high gain up,3 = low gain down,4 = high gain down. */
    hi_u8       contrast;        /* indicates the direction of contrast processing applied by the camera when the image
                                    was shot. 0 = normal,1 = soft,2 = hard */
    hi_u8       saturation;     /* indicates the direction of saturation processing applied by the camera when
                                    the image was shot. 0 = normal,1 = low saturation,2 = high saturation */
    hi_u8       sharpness;    /* indicates the direction of sharpness processing applied by the camera when the image
                                was shot. 0 = normal,1 = soft,2 = hard . */
    hi_u8       metering_mode;      /* exposure metering method. '0' means unknown, '1' average, '2' center weighted
...
typedef struct {
    ...
    hi_u32          digital_zoom_ratio; /* indicates the digital zoom ratio when the image was shot. if the
...
typedef struct {
    ...
    hi_dng_rational as_shot_neutral[CFACOLORPLANE]; /* specifies the selected white balance at time of capture,
...
typedef struct {
    ...
    hi_s32 center_x_offset;        /* RW; range: [-511, 511]; horizontal offset of the image distortion center
                                    relative to image center. */
    hi_s32 center_y_offset;        /* RW; range: [-511, 511]; vertical offset of the image distortion center relative
                                    to image center. */
    hi_s32 distortion_ratio;      /* RW; range: [-300, 500]; LDC distortion ratio.when spread on, distortion_ratio
...
typedef struct {
    ...
    hi_s32 src_cali_ratio[SRC_LENS_COEF_SEG][SRC_LENS_COEF_NUM];  /* RW; lens distortion coefficients of the source
                                                                    image, with 5 decimal numbers */
    hi_s32 src_jun_pt;                                             /* RW; junction point of the two segments */
    hi_s32 dst_cali_ratio[DST_LENS_COEF_SEG][DST_LENS_COEF_NUM];  /* RW; lens distortion coefficients, with 5 decimal
                                                                    numbers */
    hi_s32 dst_jun_pt[DST_LENS_COEF_SEG_POINT];                   /* RW; junction point of the three segments */
    hi_s32 max_du;                                                /* RW; max undistorted distance before 3rd
...
typedef struct {
    ...
    hi_s32 center_x_offset;    /* RW; range: ABS(center_x_offset)->[0, min(128, width*0.08)], horizontal offset
                                of the image distortion center relative to image center. */
    hi_s32 center_y_offset;    /* RW; range: ABS(center_x_offset)->[0, min(128, height*0.08)], vertical offset of
                                the image distortion center relative to image center. */
    hi_s32 distortion_ratio;  /* RW; range: [-300, 300], LDC distortion ratio. */
    hi_s32 min_ratio;         /* RW; range: [-300, 300], to remove the black region around when performing
...
typedef struct {
    ...
    hi_s32               center_x_offset; /* RW;range: [-511, 511];horizontal offset of the image distortion center
                                            relative to image center. */
    hi_s32               center_y_offset; /* RW;range: [-511, 511];vertical offset of the image distortion center
                                            relative to image center. */
