...
![](https://gitee.com/wgm2022/mypic/raw/master/hispark_taurus_helloworld_sample/009SAMPLE_PIPE_INFO_S.png)
...
可参考**源码的device/soc/hisilicon/hi3516dv300/sdk_linux/sample/taurus/helloworld/smp中的sample_lcd.c**中的int SampleVioVpssVoMipi(void)接口中的config vi部分。
...
* 可参考**源码的device/soc/hisilicon/hi3516dv300/sdk_linux/sample/taurus/helloworld/smp中**的**int SampleVioVpssVoMipi(void)**接口中的config vb部分。
...
* 可参考**源码的device/soc/hisilicon/hi3516dv300/sdk_linux/sample/taurus/helloworld/smp中**的sample_lcd.c中的HI_S32 **int SampleVioVpssVoMipi(void)**接口中的get picture buffer size部分和get raw buffer size部分。
...
通过**HI_S32 SAMPLE_COMM_SYS_Init(VB_CONFIG_S* pstVbConfig);**接口进行system初始化，可参考**源码的device/soc/hisilicon/hi3516dv300/sdk_linux/sample/taurus/helloworld/smp中**的sample_lcd.c中的**int SampleVioVpssVoMipi(void)**接口中的**vb init & MPI system init**部分，如下图所示：

![](https://gitee.com/wgm2022/mypic/raw/master/hispark_taurus_helloworld_sample/025vb%20init.png)

进入SAMPLE_COMM_SYS_Init接口，对其调用的底层接口进行说明和阐述，调用的接口如下：
...
通过SAMPLE_COMM_VI_StartVi接口实现启动vi功能，可参考**源码的device/soc/hisilicon/hi3516dv300/sdk_linux/sample/taurus/helloworld/smp中**的**int SampleVioVpssVoMipi(void)**接口中的start vi部分，如下图所示：

![](https://gitee.com/wgm2022/mypic/raw/master/hispark_taurus_helloworld_sample/033start%20vi.png)

针对SAMPLE_COMM_VI_StartVi调用的底层接口进行如下解释和说明：
...
该接口为初始化MIPI，这里自行查阅**源码的device/soc/hisilicon/hi3516dv300/sdk_linux/sample/platform/common/sample_comm_vi.c中**

HI_S32 SAMPLE_COMM_VI_StartMIPI(SAMPLE_VI_CONFIG_S* pstViConfig)接口。
