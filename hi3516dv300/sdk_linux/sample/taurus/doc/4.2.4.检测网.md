...
* 将/cfg目录下的resnet18.cfg按照如下方式进行修改，具体修改方式参考附录6.3。

* 上述步骤修改完成后，输入./darknet detector train hand.data cfg/resnet18.cfg即可进行训练。

**注：以上训练指令路径需在Linux环境下进行修改，并确保resnet18.cfg已经参考[附录6.1](6.1.yolov2%20resnet18.cfg%E7%BD%91%E7%BB%9C.md)修改正确。可参考 https://netron.app/ 查看网络结构和参数，部分网络结构截图如下图所示：**
...
关于Caffe环境的搭建，请参考附录6.4章节，这里不再论述。
...
```
pip3 install torchvision==0.4.0 -i https://pypi.tuna.tsinghua.edu.cn/simple some-package
...
* 步骤3：由于python的本地版本是python3.6，开源代码为 python2.X，因此需要对代码语法做适当调整
  ...
  * 将prototxt.py 按照如下方式进行修改，适配python3版本的print。
  ...
  修改后的protxt.py如附录6.5所示：

  **注：请通过对比工具仔细核对源码和附录6.5中的更改点，这里不逐一列举。**
...
* 步骤3：模型转换
  ...
  * 在Ubuntu系统的darknet2caffe目录下，执行下面的命令，创建一个resnet18.cfg文件，将本文附6.3节下的内容都复制到resnet18.cfg文件中。
  ...
  * 当转换成功后，会在darknet2caffe目录生成一个resnet18.caffemodel和一个resnet18.prototxt文件，如下图所示。
  ...
  ![](https://gitee.com/wgm2022/mypic/raw/master/hispark_taurus_nnie_sample/061%E5%BE%97%E5%88%B0caffemodel.png)
