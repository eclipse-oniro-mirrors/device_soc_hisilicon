    ...
    HI_CHAR *pchDstFileName, HI_U32 u32Width, HI_U32 u32Height)
{
    ...
    SAMPLE_CHECK_EXPR_GOTO(HI_NULL == pstCannyInfo->pFpSrc, CANNY_INIT_FAIL, "Error,Open file %s failed!\n",
        pchSrcFileName);
    pstCannyInfo->pFpDst = fopen(pchDstFileName, "wb");
    SAMPLE_CHECK_EXPR_GOTO(HI_NULL == pstCannyInfo->pFpDst, CANNY_INIT_FAIL, "Error,Open file %s failed!\n",
        pchDstFileName);
