...
1. sample适用芯片类型
   Hi3559V200/Hi3556AV100/Hi3559V200/Hi3556V200/Hi3516DV300/Hi3516CV500
...
2. LCD屏依赖(不支持触摸功能)
  默认支持320 * 240分辨率的LCD屏(横屏)，其他分辨率LCD屏需要适配，适配请参考sample_hifb.h文件，修改其中的屏幕及屏幕分辨率相关信息。
...
2. 触摸屏依赖
  默认支持768 * 400分辨率的触摸屏(竖屏)，其他分辨率触摸屏需要适配，适配请参考sample_utils.h文件，修改其中的屏幕及屏幕分辨率相关信息。

3. 加载驱动
（1）客户根据实际情况适配触摸屏驱动和LCD屏幕驱动程序，需要在系统启动后使用insmod命令加载。
（2）依赖的其他驱动包含TDE, HIFB和VO的驱动KO文件，这些驱动单板启动后系统默认会加载，不需单独加载。
...
5. 编译Sample
（1）进入到middleware/sample/higv/softkb_sample目录下面，执行make命令，编译生成softkb_sample可执行文件。
（2）把softkb_sample, higv.bin, res目录，拷贝到单板的某个位置
（3）使能可执行文件，chmod +x *
（4）执行softkb_sample文件，即可看到屏幕有图像显示。

6. sample运行
  softkb_sample运行后，在首界面显示的是edit控件、输入法窗口和虚拟键盘窗口，表示sample已经运行成功。
