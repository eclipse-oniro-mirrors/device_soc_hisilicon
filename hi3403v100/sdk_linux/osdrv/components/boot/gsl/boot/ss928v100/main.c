...
#define SS928_BOOT_ARRAY_INDEX_3               3
#define SS928_BOOT_HPM_ARRAY_SIZE              4
#define SS928_BOOT_HPM_MASK                    0x3ff
#define SS928_BOOT_SHIFT_1                     1
#define SS928_BOOT_SHIFT_2                     2
#define SS928_BOOT_SHIFT_4                     4
#define SS928_BOOT_SHIFT_8                     8
#define SS928_BOOT_SHIFT_16                    16
#define SS928_BOOT_DELTA_SIGN_CORE_MASK        0x800
#define SS928_BOOT_DELTA_SIGN_MDA_MASK         0x80
#define SS928_BOOT_DELTA_SIGN_NPU_MASK         0x8
#define SS928_BOOT_DELTA_VALUE_MASK            0x7
#define SS928_BOOT_DIVISOR_10                  10
#define SS928_BOOT_DIVISOR_100                 100
#define SS928_BOOT_FACTOR_10                   10
#define SS928_BOOT_TEMPERATURE_INDEX_0P5       0.5f
#define SS928_BOOT_TEMPERATURE_SENSOR_COUNT    3
#define SS928_BOOT_TEMPERATURE_REG_STEP        0x100
#define SS928_BOOT_TEMPERATURE_DIVISOR         718
#define SS928_BOOT_TEMPERATURE_MULTIPLIER      165
#define SS928_BOOT_TEMPERATURE_OFFSET          40
#define SS928_BOOT_TEMPERATURE_HIGH            100
#define SS928_BOOT_TEMPERATURE_MIDDLE          70
#define SS928_BOOT_TEMPERATURE_BASE            146
#define SS928_BOOT_ADJUST_LOW_NPU              2
#define SS928_BOOT_ADJUST_LOW_MDA              3
#define SS928_BOOT_ADJUST_LOW_CORE             4
#define SS928_BOOT_ADJUST_HIGH_NPU             5
#define SS928_BOOT_ADJUST_HIGH_MDA             5
#define SS928_BOOT_ADJUST_HIGH_CORE            8
#define SS928_BOOT_HPM_CORE_INCREMENT          59
#define SS928_BOOT_HPM_NPU_INCREMENT           62
#define SS928_BOOT_HPM_MDA_INCREMENT           61
#define SS928_BOOT_DDRCA_ENABLE                5
#define SS928_BOOT_DDRC_CHANNEL_NUM            4
#define SS928_BOOT_DDRC_ENABLE                 1
#define SS928_BOOT_DDRC_DISABLE                0
#define SS928_BOOT_DDRC_VALID_MASK             0xf
...
static unsigned hpm_value_avg(const unsigned int *val, unsigned int num)
{
    ...
    return (tmp / CYCLE_NUM) >> SS928_BOOT_SHIFT_2;
}
    ...
    unsigned int *hpm_mda)
{
    int i;
    unsigned int temp;
    unsigned int core_value[SS928_BOOT_HPM_ARRAY_SIZE] = {0, 0, 0, 0};
    unsigned int mda_value[SS928_BOOT_HPM_ARRAY_SIZE] = {0, 0, 0, 0};
    unsigned int npu_value[SS928_BOOT_HPM_ARRAY_SIZE] = {0, 0, 0, 0};
    ...
    for (i = 0; i < CYCLE_NUM; i++) {
        ...
        npu_value[1] += (temp >> SS928_BOOT_SHIFT_16) & SS928_BOOT_HPM_MASK;
        npu_value[0] += temp & SS928_BOOT_HPM_MASK;
        temp = readl(SYSCTRL_REG + HPM_NPU_REG1);
        npu_value[SS928_BOOT_ARRAY_INDEX_3] += (temp >> SS928_BOOT_SHIFT_16) & SS928_BOOT_HPM_MASK;
        npu_value[SS928_BOOT_ARRAY_INDEX_2] += temp & SS928_BOOT_HPM_MASK;

        /* mda */
        temp = readl(SYSCTRL_REG + HPM_MDA_REG0);
        mda_value[1] += (temp >> SS928_BOOT_SHIFT_16) & SS928_BOOT_HPM_MASK;
        mda_value[0] += temp & SS928_BOOT_HPM_MASK;
        temp = readl(SYSCTRL_REG + HPM_MDA_REG1);
        mda_value[SS928_BOOT_ARRAY_INDEX_3] += (temp >> SS928_BOOT_SHIFT_16) & SS928_BOOT_HPM_MASK;
        mda_value[SS928_BOOT_ARRAY_INDEX_2] += temp & SS928_BOOT_HPM_MASK;

        /* core */
        temp = readl(SYSCTRL_REG + HPM_CORE_REG0);
        core_value[1] += (temp >> SS928_BOOT_SHIFT_16) & SS928_BOOT_HPM_MASK;
        core_value[0] += temp & SS928_BOOT_HPM_MASK;
        temp = readl(SYSCTRL_REG + HPM_CORE_REG1);
        core_value[SS928_BOOT_ARRAY_INDEX_3] += (temp >> SS928_BOOT_SHIFT_16) & SS928_BOOT_HPM_MASK;
        core_value[SS928_BOOT_ARRAY_INDEX_2] += temp & SS928_BOOT_HPM_MASK;
    }

    *hpm_core = hpm_value_avg(core_value, SS928_BOOT_HPM_ARRAY_SIZE);
    *hpm_mda = hpm_value_avg(mda_value, SS928_BOOT_HPM_ARRAY_SIZE);
    *hpm_npu = hpm_value_avg(npu_value, SS928_BOOT_HPM_ARRAY_SIZE);
}
...
static void set_hpm_core_volt(unsigned int hpm_core_value, int delta_v)
{
    ...
    volt_val = (CORE_CURVE_B - CORE_CURVE_A * hpm_core_value) / SS928_BOOT_DIVISOR_10;
...
static void set_hpm_mda_volt(unsigned int hpm_mda_value, int delta_v)
{
    ...
    volt_val = (MEDIA_CURVE_B - MEDIA_CURVE_A * hpm_mda_value) / SS928_BOOT_DIVISOR_10;
...
static void set_hpm_npu_volt(unsigned int hpm_npu_value, int delta_v)
{
    ...
    volt_val = (NPU_CURVE_B - NPU_CURVE_A * hpm_npu_value) / SS928_BOOT_DIVISOR_100;
...
static void get_delta_v(int *core_delta_v, int *npu_delta_v,  int *mda_delta_v)
{
     ...
     */
    int flag =  value & SS928_BOOT_DELTA_SIGN_CORE_MASK ? -1 : 1;
    *core_delta_v = flag * (int)((value >> SS928_BOOT_SHIFT_8) & SS928_BOOT_DELTA_VALUE_MASK) *
        SS928_BOOT_FACTOR_10;

    /* mda:bit 7-4,
     * bit7 equal to 1 means negative, equal to 0 means positive,
     * bit 4-6 is the  absolute delta_v
     */
    flag =  value & SS928_BOOT_DELTA_SIGN_MDA_MASK ? -1 : 1;
    *mda_delta_v = flag * (int)((value >> SS928_BOOT_SHIFT_4) & SS928_BOOT_DELTA_VALUE_MASK) *
        SS928_BOOT_FACTOR_10;
     ...
     */
    flag =  value & SS928_BOOT_DELTA_SIGN_NPU_MASK ? -1 : 1;
    *npu_delta_v = flag * (int)(value & SS928_BOOT_DELTA_VALUE_MASK) * SS928_BOOT_FACTOR_10;
}
...
static void get_temperature(int *temperature)
{
    int value = 0;
    int tsensor_chn;
    float m = SS928_BOOT_TEMPERATURE_INDEX_0P5;
    float temperature_temp[SS928_BOOT_TEMPERATURE_SENSOR_COUNT] = {0};
    for (tsensor_chn = 0; tsensor_chn < SS928_BOOT_TEMPERATURE_SENSOR_COUNT; tsensor_chn++) {
        value = (int)(readl(REG_TSENSOR_CTRL + TSENSOR_STATUS0 +
            SS928_BOOT_TEMPERATURE_REG_STEP * tsensor_chn) & SS928_BOOT_HPM_MASK);

        m = ((float)value - SS928_BOOT_TEMPERATURE_BASE) / SS928_BOOT_TEMPERATURE_DIVISOR *
            SS928_BOOT_TEMPERATURE_MULTIPLIER - SS928_BOOT_TEMPERATURE_OFFSET;

        if (m > 0) {
            temperature_temp[tsensor_chn] = m + SS928_BOOT_TEMPERATURE_INDEX_0P5;
        } else {
            temperature_temp[tsensor_chn] = m - SS928_BOOT_TEMPERATURE_INDEX_0P5;
        }
    }

    *temperature = (temperature_temp[0] + temperature_temp[1] + temperature_temp[SS928_BOOT_ARRAY_INDEX_2]) /
        SS928_BOOT_TEMPERATURE_SENSOR_COUNT;
    ...
    unsigned int *hpm_npu, int temperature)
{
    ...
    if (temperature <= 0) {
        *hpm_npu  -= SS928_BOOT_ADJUST_LOW_NPU;
        *hpm_mda  -= SS928_BOOT_ADJUST_LOW_MDA;
        *hpm_core -= SS928_BOOT_ADJUST_LOW_CORE;
    } else if (temperature > SS928_BOOT_TEMPERATURE_HIGH) {
        *hpm_npu  += SS928_BOOT_ADJUST_HIGH_NPU;
        *hpm_mda  += SS928_BOOT_ADJUST_HIGH_MDA;
        *hpm_core += SS928_BOOT_ADJUST_HIGH_CORE;
    } else if (temperature > SS928_BOOT_TEMPERATURE_MIDDLE) {
        *hpm_npu  += SS928_BOOT_ADJUST_LOW_MDA;
        *hpm_mda  += SS928_BOOT_ADJUST_LOW_MDA;
        *hpm_core += SS928_BOOT_ADJUST_LOW_CORE;
    }

    if (otp_hpm_core) {
        if (*hpm_core > (otp_hpm_core + SS928_BOOT_HPM_CORE_INCREMENT)) {
            *hpm_core = otp_hpm_core + SS928_BOOT_HPM_CORE_INCREMENT;
        }
    }

    if (otp_hpm_npu) {
        if (*hpm_npu > (otp_hpm_npu + SS928_BOOT_HPM_NPU_INCREMENT)) {
            *hpm_npu = otp_hpm_npu + SS928_BOOT_HPM_NPU_INCREMENT;
        }
    }

    if (otp_hpm_mda) {
        if (*hpm_mda > (otp_hpm_mda + SS928_BOOT_HPM_MDA_INCREMENT)) {
            *hpm_mda = otp_hpm_mda + SS928_BOOT_HPM_MDA_INCREMENT;
        }
    }
}
...
void init_temperature(void)
{
    ...
    for (tsensor_chn = 0; tsensor_chn < SS928_BOOT_TEMPERATURE_SENSOR_COUNT; tsensor_chn++) {
        writel(TSENSOR_CTRL0_CFG, REG_TSENSOR_CTRL +
            TSENSOR_CTRL0 + SS928_BOOT_TEMPERATURE_REG_STEP * tsensor_chn);
        writel(TSENSOR_CTRL1_CFG, REG_TSENSOR_CTRL +
            TSENSOR_CTRL1 + SS928_BOOT_TEMPERATURE_REG_STEP * tsensor_chn);
    }
}
