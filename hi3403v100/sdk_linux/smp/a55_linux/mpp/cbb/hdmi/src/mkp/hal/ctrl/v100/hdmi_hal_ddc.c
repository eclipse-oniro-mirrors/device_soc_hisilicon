...
static td_s32 ddc_access_enable_wait(td_u32 timeout)
{
    ...
    hdmi_reg_cpu_ddc_req_set(TD_TRUE);
...
static td_s32 ddc_access_disable_wait(td_u32 timeout)
{
    ...
    hdmi_reg_cpu_ddc_req_set(TD_FALSE);
...
static td_s32 ddc_sda_wait(td_u32 timeout)
{
    ...
    if (!hdmi_reg_ddc_sda_st_get()) {
        hdmi_reg_dcc_man_en_set(TD_TRUE);
        while ((!hdmi_reg_ddc_sda_st_get()) && tmp_timeout < timeout) {
            tmp_timeout++;

            /* pull scl high */
            hdmi_reg_ddc_scl_oen_set(TD_TRUE);
            osal_udelay(DDC_DEFAULT_DELAY);
            /* pull scl low */
            hdmi_reg_ddc_scl_oen_set(TD_FALSE);
            osal_udelay(DDC_DEFAULT_DELAY);
        }
        ...
        if (tmp_timeout < timeout && (hdmi_reg_ddc_sda_st_get())) {
            /* pull sda low */
            hdmi_reg_ddc_sda_oen_set(TD_FALSE);
            osal_udelay(DDC_DEFAULT_DELAY);
            /* pull scl high */
            hdmi_reg_ddc_scl_oen_set(TD_TRUE);
            osal_udelay(DDC_DEFAULT_DELAY);
            /* pull sda high */
            hdmi_reg_ddc_sda_oen_set(TD_TRUE);
            osal_udelay(DDC_DEFAULT_DELAY);
        ...
        hdmi_reg_dcc_man_en_set(TD_FALSE);
    }
...
static td_s32 ddc_cmd_issue(const ddc_cfg *cfg)
{
    td_u16 data_size;
    td_u8 slave_addr;
    td_u8 segment;
    ...
    if (cfg->master_mode == DDC_MASTER_MODE_PWD) {
        ...
        osal_udelay(DDC_DEFAULT_DELAY);
        hdmi_reg_pwd_mst_cmd_set((td_u8)cfg->issue_mode);
    } else {
...
static td_s32 ddc_read(const ddc_cfg *cfg)
{
    td_u32 len;
    ddc_func_type type;
    td_u8 *data = TD_NULL;
...
static td_s32 ddc_write(const ddc_cfg *cfg)
{
    td_u32 len;
    ddc_func_type type;
    td_u8 *data = TD_NULL;
...
static td_s32 ddc_data_pwd_issue(const ddc_cfg *cfg, td_bool read_issue)
{
    td_s32 data_size;
...
static td_s32 ddc_issue(const ddc_cfg *cfg, ddc_record_elem *tmp_elem)
{
    td_s32 ret = TD_SUCCESS;
    ...
    /* issue command */
    if (ddc_cmd_issue(cfg) != TD_SUCCESS) {
...
static td_s32 edid_get_extern_blk(hdmi_device_id hdmi, td_u8 ext_blk_num, td_s32 raw_len, ddc_cfg *cfg)
{
    td_s32 ret = raw_len;
...
td_s32 hal_hdmi_ddc_issue(hdmi_device_id hdmi, const ddc_cfg *cfg)
{
    ...
    hdmi_if_null_warn_return(cfg, OT_ERR_HDMI_NULL_PTR);
    hdmi_if_null_warn_return(tmp, OT_ERR_HDMI_NULL_PTR);
    hdmi_if_false_warn_return(tmp->run.init, OT_ERR_HDMI_NOT_INIT);
...
td_s32 hal_hdmi_ddc_edid_raw_get(hdmi_device_id hdmi, td_s32 size, td_u8 *data)
{
    ...
    hdmi_if_null_warn_return(data, OT_ERR_HDMI_NULL_PTR);
