...
static hdmi_hal_cfg g_hal_cfg[HDMI_ID_MAX];
...
static td_void hal_hdmi_cbar_enable(td_bool enable)
{
    td_u32 *reg_addr = TD_NULL;
    td_u32  reg_value;

    reg_addr = (td_u32 *)osal_ioremap_nocache(HDMI_COLOR_BAR_BASE, 4); /* 4: register size */
    ...
    osal_iounmap((void *)reg_addr, 4); /* 4: register size */
}
#endif
...
static td_void hal_hdmi_hardware_init(const struct hdmi_hal_ *hal)
{
    drv_hdmi_prod_crg_init(hal->hal_ctx.hdmi_id);
    return;
}

static td_void hal_hdmi_tmds_mode_set(const struct hdmi_hal_ *hal, hdmi_tmds_mode tmds_mode)
{
...
static td_void hal_hdmi_avmute_set(const struct hdmi_hal_ *hal, td_bool avmute)
{
...
static td_void hal_hdmi_emi_status_get(const struct hdmi_hal_ *hal, hdmi_emi_status *emi_status)
{
    hdmi_phy_info *phy_info = TD_NULL;

    hdmi_if_fpga_return_void();
    hdmi_if_null_return_void(emi_status);

    phy_info = (hdmi_phy_info *)osal_kmalloc(sizeof(hdmi_phy_info), osal_gfp_atomic);
    if (phy_info == TD_NULL) {
        hdmi_err("alloc hdmi_phy_info struct memory fail\n");
        return;
    }

    (td_void)memset_s(phy_info, sizeof(hdmi_phy_info), 0, sizeof(hdmi_phy_info));
    hal_hdmi_phy_info_get(hal->hal_ctx.hdmi_id, phy_info);

    emi_status->sw_emi_enable = phy_info->ssc_cfg.ssc_enable;
    emi_status->debug_enable  = phy_info->ssc_cfg.ssc_debug_en;
    hal_hdmi_phy_ssc_get(hal->hal_ctx.hdmi_id, &emi_status->hw_emi_enable);
...
static td_void hal_hdmi_csc_param_set(struct hdmi_hal_ *hal, const hdmi_video_config *video_cfg)
{
...
static td_void hal_hdmi_infoframe_set(const struct hdmi_hal_ *hal,
    hdmi_infoframe_id infoframe_id, const td_u8 *in_buffer, td_u32 buf_len)
{
    hdmi_if_null_return_void(hal);
...
static td_void hal_hdmi_infoframe_enable_set(const struct hdmi_hal_ *hal,
...
static td_s32 hal_hdmi_video_path_set(const struct hdmi_hal_ *hal, const hdmi_video_config *video_cfg)
{
    td_s32 ret;
    hdmi_video_path video_path = {0};
    hdmi_hal_cfg *hal_cfg = TD_NULL;

    hdmi_if_null_return(hal, OT_ERR_HDMI_NULL_PTR);
    hdmi_if_null_return(video_cfg, OT_ERR_HDMI_NULL_PTR);

    hal_cfg = intf_hal_info_ptr_get(hal->hal_ctx.hdmi_id);
    hdmi_if_null_return(hal_cfg, OT_ERR_HDMI_NULL_PTR);
...
static td_void hal_hdmi_ctrl_reset(const struct hdmi_hal_ *hal)
{
    td_u32 i;
    ...
    hal_hdmi_phy_oe_get(hal->hal_ctx.hdmi_id, &output);
    if (output == TD_TRUE) {
        hdmi_info("oe enable, do not reset!\n");
        return;
    }

    hal_hdmi_ctrl_data_reset(hal->hal_ctx.hdmi_id, TD_FALSE, 0);
...
static td_s32 hal_hdmi_phy_hw_spec_set(const struct hdmi_hal_ *hal, td_u32 tmds_clk, const hdmi_hw_spec *hw_spec)
{
    hdmi_if_fpga_return(TD_SUCCESS);
    hdmi_if_null_return(hw_spec, OT_ERR_HDMI_NULL_PTR);
    return hal_hdmi_phy_params_set(hal->hal_ctx.hdmi_id, tmds_clk, hw_spec);
}

static td_s32 hal_hdmi_phy_hw_spec_get(const struct hdmi_hal_ *hal, hdmi_hw_spec *hw_spec)
{
    td_s32 ret;
    ...
    hdmi_if_null_return(hw_spec, OT_ERR_HDMI_NULL_PTR);
    ret = hal_hdmi_phy_params_get(hal->hal_ctx.hdmi_id, &phy_hw_param);
...
static td_void hal_hdmi_phy_output_enable_set(const struct hdmi_hal_ *hal, td_bool enable)
{
    hdmi_if_fpga_return_void();
    hdmi_if_null_return_void(hal);
    hal_hdmi_phy_oe_set(hal->hal_ctx.hdmi_id, enable);

    return;
}

static td_void hal_hdmi_phy_power_enable_set(const struct hdmi_hal_ *hal, td_bool enable)
{
    td_bool old_eable = TD_FALSE;

    hdmi_if_fpga_return_void();
    hal_hdmi_phy_power_get(hal->hal_ctx.hdmi_id, &old_eable);
    if (enable != old_eable) {
        hal_hdmi_phy_power_set(hal->hal_ctx.hdmi_id, enable);
    }

    return;
}

static td_void hal_hdmi_phy_set(const struct hdmi_hal_ *hal, const hdmi_phy_cfg *phy_cfg)
{
    ...
    hal_hdmi_phy_tmds_set(hal->hal_ctx.hdmi_id, &cfg);

    return;
}

static td_void hal_hdmi_tx_capability_get(const struct hdmi_hal_ *hal, hdmi_tx_capability_data *tx_capability)
{
    errno_t ret;
    hdmi_hal_cfg *hal_cfg = TD_NULL;
    ...
    ret = memcpy_s(tx_capability, sizeof(*tx_capability), &hal_cfg->tx_capability, sizeof(hdmi_tx_capability_data));
    hdmi_unequal_eok_return_void(ret);

    return;
}

static td_void hal_hdmi_sequencer_handler_process(const struct hdmi_hal_ *hal)
{
    hal_hdmi_mach_invoke(hal->hal_ctx.hdmi_id);
    return;
}

static td_void hal_hdmi_hw_audio_status_get(td_u32 hdmi_id, hdmi_hardware_status *hw_status)
{
    ...
    hw_status->audio_status.down_sample  = TD_FALSE;
...
static td_void hal_hdmi_hw_video_status_get(td_u32 hdmi_id, hdmi_hardware_status *hw_status)
{
    ...
    hw_status->video_status.ycbcr420_422    = TD_FALSE; // not support
    hw_status->video_status.ycbcr422_444    = TD_FALSE; // not support
    hw_status->video_status.in420_ydemux    = video_stat.dwsm_vert_enable;
    hw_status->video_status.out420_ydemux   = TD_FALSE; // not support
    ...
    hw_status->video_status.sync_pol        = TD_FALSE; // not support
    hw_status->video_status.de_pol          = video_stat.out_hv_sync_pol.de_inver;
    hw_status->video_status.swap_hs_cs      = TD_FALSE; // not support
...
static td_void hal_hdmi_phy_hwparam_get(td_u32 hdmi_id, hdmi_hwspec *hwspec)
{
    errno_t ret;

    hdmi_phy_hw_param hw_param = {0};

    hal_hdmi_phy_params_get(hdmi_id, &hw_param);
    ret = memcpy_s(&hwspec->hwspec_user, sizeof(hwspec->hwspec_user),
        &hw_param.spec_user_set, sizeof(hdmi_hw_spec));
    if (ret != EOK) {
        hdmi_err("secure function error:%d\n", ret);
        return;
    }
...
static td_void hal_hdmi_hardware_status_get(const struct hdmi_hal_ *hal, hdmi_hardware_status *hw_status)
{
    ...
    hal_hdmi_phy_oe_get(hal->hal_ctx.hdmi_id, &hw_status->phy_status.phy_oe);
    hal_hdmi_phy_power_get(hal->hal_ctx.hdmi_id, &hw_status->phy_status.phy_power_on);
...
static td_void hal_hdmi_hot_plug_status_get(const struct hdmi_hal_ *hal, td_bool *hot_plug)
{
    hdmi_hpd_rsen hpd_rsen = {0};
...
static td_s32 hal_hdmi_audio_path_set(const struct hdmi_hal_ *hal, const hdmi_audio_config *audio_cfg)
{
    hdmi_audio_path audio_path = {0};

    hdmi_if_null_return(hal, OT_ERR_HDMI_NULL_PTR);
    hdmi_if_null_return(audio_cfg, OT_ERR_HDMI_NULL_PTR);
...
static td_void hal_hdmi_audio_path_enable_set(const struct hdmi_hal_ *hal, td_bool enable)
{
    hdmi_if_null_return_void(hal);
...
static td_s32 hal_hdmi_edid_raw_data_read(const struct hdmi_hal_ *hal, td_u32 size, td_u8 out_buffer[])
{
    td_s32 ret;

    hdmi_if_null_return(hal, OT_ERR_HDMI_NULL_PTR);
    hdmi_if_null_return(out_buffer, OT_ERR_HDMI_NULL_PTR);
    ret = hal_hdmi_ddc_edid_raw_get(hal->hal_ctx.hdmi_id, size, out_buffer);

    return ret;
}

static td_void hal_hdmi_phy_output_enable_get(const struct hdmi_hal_ *hal, td_bool *enable)
{
    hal_hdmi_phy_oe_get(hal->hal_ctx.hdmi_id, enable);
    return;
}

static td_void hal_hdmi_video_mute_set(const struct hdmi_hal_ *hal, td_bool v_mute)
{
    hdmi_if_null_return_void(hal);
...
static td_void hal_hdmi_black_data_set(const struct hdmi_hal_ *hal, const hdmi_black_frame_info *black)
{
    hdmi_if_null_return_void(hal);
...
#ifdef HDMI_DEBUG_SUPPORT
static td_void hal_hdmi_debug(const struct hdmi_hal_ *hal, hdmi_debug_cmd debug_cmd, const td_void *data)
{
    ...
    switch (debug_cmd) {
        case HDMI_DEBUG_CMD_COLOR_BAR:
            hal_hdmi_cbar_enable(*(td_bool *)data);
            break;
...
static td_u32 *hal_hdmi_base_addr_get(const struct hdmi_hal_ *hal)
{
    td_u32 *base_addr = TD_NULL;
    ...
    base_addr = (td_u32 *)hal->hal_ctx.base_addr;

    return base_addr;
}

#ifdef HDMI_SCDC_SUPPORT
static td_void hal_hdmi_scdc_status_set(const struct hdmi_hal_ *hal, const hdmi_scdc_status *status)
{
    scdc_attr attr = {0};
    ...
    attr.tmds_clk_ratio40x = (status->tmds_bit_clk_ratio == SCDC_TMDS_BIT_CLK_RATIO_40X) ? TD_TRUE : TD_FALSE;
...
static td_void hal_hdmi_scdc_status_get(const struct hdmi_hal_ *hal, hdmi_scdc_status *status)
{
    scdc_attr attr = {0};
...
td_s32 hal_hdmi_open(const hdmi_hal_init *hal_init, hdmi_hal **hal_handle)
{
    ...
    hal_hdmi_mach_init(hal->hal_ctx.hdmi_id);
...
void hal_hdmi_close(struct hdmi_hal_ *hal)
{
    ...
    hal_hdmi_mach_deinit(hal->hal_ctx.hdmi_id);
