 ...
 */
#ifndef HDMI_HAL_SCDC_H
#define HDMI_HAL_SCDC_H
...
typedef union {
    struct {
        td_u8 ch_error_cnt_l : 8;
        td_u8 ch_error_cnt_h : 7;
        td_u8 ch_valid       : 1;
    } u16;
    td_u16 word;
...
typedef union {
    struct {
        td_u8 test_read_request_delay : 7;
        td_u8 test_read_request       : 1;
    } u8;
    td_u8 byte;
} scdc_rr_test_cfg;
...
typedef struct {
    td_u8            sink_version; // SCDC_OFFSET_SINK_VERSION
    td_u8            src_version;  // SCDC_OFFSET_SRC_VERSION
    ...
    td_u8            ieee_out[SCDC_OFFSET_IEEE_OUI_1ST - SCDC_OFFSET_IEEE_OUI_3TH + 1];
    td_u8            device_id[SCDC_OFFSET_DEVICE_ID_TAIL - SCDC_OFFSET_DEVICE_ID_HEAD + 1];
...
typedef struct {
    ...
    td_u8                 err_chk_sum;     // SCDC_OFFSET_DRR_stb_det_CHKSUM
} scdc_updata_status;

typedef struct {
    td_bool sink_read_quest;
    /* TD_TRUE indicate enable; TD_FALSE indicate disable. */
    td_bool src_scramble;
    /* TD_TRUE indicate enable; TD_FALSE indicate disable. */
    td_bool sink_scramble;
    /* TD_TRUE indicate 40x; TD_FALSE indicate 10x. */
...
typedef struct {
    td_u32 sramble_fail_cnt;
    td_u32 oe_disable_fail_cnt;
    td_u32 oe_enable_fail_cnt;
    td_u32 sink_un_read_cnt;
} scdc_error_cnt;
