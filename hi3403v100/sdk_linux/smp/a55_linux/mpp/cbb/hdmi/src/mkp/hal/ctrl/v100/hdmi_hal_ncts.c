...
{
    td_u32 i;
    td_u32 tmp_cts = NCTS_INVALID_DATA;
    td_u32 tmp_n   = NCTS_INVALID_DATA;
    hdmi_audio_cts_n *audio_cts_n = &g_audio_cts_n[0];
     ...
     *      (128 * samp_rate)
     */
    if (tmp_cts == 0 && sample_rate) {
        tmp_cts = (tmp_n / NCTS_CALC_FACTOR) * tmds_clk * HDMI_DECIMAL / (sample_rate / HDMI_HUNDRED);
    } else if (tmp_cts == NCTS_INVALID_DATA && sample_rate) { /* can't find, N default 48k samprate */
