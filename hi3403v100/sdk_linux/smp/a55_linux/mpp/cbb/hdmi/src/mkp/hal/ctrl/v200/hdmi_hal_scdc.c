...
static td_s32 scdc_scramble_set(hdmi_device_id hdmi_id, const scdc_scramble *scramble)
{
    ...
    /* source scramble */
    hdmi_reg_enc_scr_on_set(hdmi_id, scramble->src_scramble);
    /* sink scramble */
...
static td_s32 scdc_scramble_get(hdmi_device_id hdmi_id, scdc_scramble *scramble)
{
    ...
    /* source scrambled */
    scramble->src_scramble = hdmi_reg_enc_scr_on_get(hdmi_id) ? TD_TRUE : TD_FALSE;
...
static td_void scdc_oe_set(td_u32 id, td_bool oe_en, td_u32 timeout, scdc_infomation *info)
{
    td_bool oe_enable = TD_FALSE;

#ifndef HDMI_FPGA_SUPPORT
    hal_hdmi_phy_oe_set(id, oe_en);
    osal_msleep(timeout);
    hal_hdmi_phy_oe_get(id, &oe_enable);
#else
    ot_unused(id);
    ot_unused(timeout);
#endif
...
td_s32 hal_hdmi_scdc_init(hdmi_device_id hdmi_id)
{
    ...
    if (!scdc_info->status.init) {
        ...
        ret = hal_hdmi_mach_register(hdmi_id, &scdc_info->status.mach_id, &mach_ctrl);
    }
...
td_s32 hal_hdmi_scdc_deinit(hdmi_device_id hdmi_id)
{
    ...
    ret = hal_hdmi_mach_unregister(hdmi_id, scdc_info->status.mach_id);
    (td_void)memset_s(scdc_info, sizeof(scdc_infomation), 0, sizeof(scdc_infomation));

    return ret;
}

static td_s32 scdc_scramble_try_once(hdmi_device_id hdmi_id, scdc_infomation *scdc_info,
    scdc_scramble *scramble, td_u32 timeout)
{
    scdc_oe_set(hdmi_id, TD_FALSE, SCDC_OE_DISABLE_WAIT_TIME, scdc_info);
    ...
    osal_msleep(timeout);
#endif
    scdc_oe_set(hdmi_id, TD_TRUE, timeout, scdc_info);
...
td_s32 hal_hdmi_scdc_attr_set(hdmi_device_id hdmi_id, scdc_attr *attr)
{
    ...
    do {
        ret = scdc_scramble_try_once(hdmi_id, scdc_info, &scramble, timeout);
        timeout += scdc_info->attr.scramble_interval;
    ...
    if ((ret != TD_SUCCESS) || scdc_info->attr.sink_scramble == TD_FALSE) {
        ret += hal_hdmi_mach_stop(hdmi_id, scdc_info->status.mach_id);
