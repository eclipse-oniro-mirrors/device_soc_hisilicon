...
static td_void hal_hdmi_csc_param_set(struct hdmi_hal_ *hal, const hdmi_video_config *video_cfg)
{
    ...
    hal->hal_ctx.video_cfg.out_csc_quantization = video_cfg->out_csc_quantization;
...
#ifdef HDMI_PRODUCT_SS928V100
    ret = hal_hdmi_phy_params_set(hal->hal_ctx.hdmi_id, tmds_clk, hw_spec);
#else
    ot_unused(hal);
...
#endif
    ret = memcpy_s(hw_spec, sizeof(hdmi_hw_spec), &phy_hw_param.spec_drv_use, sizeof(phy_hw_param.spec_drv_use));
...
static td_void hal_hdmi_phy_hwparam_get(td_u32 hdmi_id, hdmi_hwspec *hwspec)
{
    ...
    ret = memcpy_s(&hwspec->hwspec_user, sizeof(hwspec->hwspec_user),
        &hw_param.spec_user_set, sizeof(hw_param.spec_user_set));
...
static td_void hal_hdmi_hdp_intr_status_get(const struct hdmi_hal_ *hal, td_bool *intr_status)
{
    ot_unused(hal);
    *intr_status = (hdmi_reg_aon_intr_stat0_get() == 0) ? TD_FALSE : TD_TRUE;
    return;
}
