...
static td_s32 drv_blit_submit_job(const ot_tde_export_func *, const gfbg_blit_opt *, const td_s32 *,
    drv_tde_surface *, drv_tde_surface *);
                             ...
                             const gfbg_blit_opt *opt, const ot_tde_export_func *tde_export_func)
{
    ...
    */
    ...
    /* submit tde the job */
    ret = drv_blit_submit_job(tde_export_func, opt, handle, single_src->src_surface, single_src->dst_surface);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
static td_s32 drv_blit_submit_job(const ot_tde_export_func *tde_export_func, const gfbg_blit_opt *opt,
                                  const td_s32 *handle, drv_tde_surface *src_surface, drv_tde_surface *dst_surface)
{
    ...
    if (opt->call_back) {
        ...
        param->src_addr = src_surface->phys_addr;
        param->dst_addr = dst_surface->phys_addr;
                     ...
                     td_bool is_refresh_screen)
{
    ...
    if (gfbg_blit_check((ot_tde_export_func **)&tde_export_func, src_img, dst_img, opt) != TD_SUCCESS) {
        gfbg_error("gfbg_blit_check failed!\n");
        return TD_FAILURE;
    }
    /* src init */
    ...
    if (opt->region_deflicker) {
        ...
        if (ret != TD_SUCCESS) {
            gfbg_error("drv_tde_module_enable_region_deflicker failed!\n");
            return ret;
        }
    }
    /* blit start */
