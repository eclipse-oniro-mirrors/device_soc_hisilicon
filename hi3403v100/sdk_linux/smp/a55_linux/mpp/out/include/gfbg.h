...
#define FBIOGET_COLORKEY_GFBG       OSAL_IOR(IOC_TYPE_GFBG, 90, ot_fb_colorkey)
/* To set the colorkey of an overlay layer */
#define FBIOPUT_COLORKEY_GFBG       OSAL_IOW(IOC_TYPE_GFBG, 91, ot_fb_colorkey)
/* To get the alpha of an overlay layer */
#define FBIOGET_ALPHA_GFBG          OSAL_IOR(IOC_TYPE_GFBG, 92, ot_fb_alpha)
/* To set the alpha of an overlay layer */
#define FBIOPUT_ALPHA_GFBG          OSAL_IOW(IOC_TYPE_GFBG, 93, ot_fb_alpha)
/* To get the origin of an overlay layer on the screen */
#define FBIOGET_SCREEN_ORIGIN_GFBG  OSAL_IOR(IOC_TYPE_GFBG, 94, ot_fb_point)
/* To set the origin of an overlay layer on the screen */
#define FBIOPUT_SCREEN_ORIGIN_GFBG  OSAL_IOW(IOC_TYPE_GFBG, 95, ot_fb_point)
/* To obtain the anti-flicker setting of an overlay layer */
#define FBIOGET_DEFLICKER_GFBG       OSAL_IOR(IOC_TYPE_GFBG, 98, ot_fb_deflicker)
/* To set the anti-flicker setting of an overlay layer */
#define FBIOPUT_DEFLICKER_GFBG       OSAL_IOW(IOC_TYPE_GFBG, 99, ot_fb_deflicker)
/* To wait for the vertical blanking region of an overlay layer */
#define FBIOGET_VER_BLANK_GFBG         OSAL_IO(IOC_TYPE_GFBG, 100)
/* To set the display state of an overlay layer */
#define FBIOPUT_SHOW_GFBG           OSAL_IOW(IOC_TYPE_GFBG, 101, td_bool)
/* To obtain the display state of an overlay layer */
#define FBIOGET_SHOW_GFBG           OSAL_IOR(IOC_TYPE_GFBG, 102, td_bool)
/* to obtain the capability of an overlay layer */
#define FBIOGET_CAPABILITY_GFBG    OSAL_IOR(IOC_TYPE_GFBG, 103, ot_fb_capability)
/* set the screen output size */
#define FBIOPUT_SCREEN_SIZE          OSAL_IOW(IOC_TYPE_GFBG, 130, ot_fb_size)
/* get the screen output size */
#define FBIOGET_SCREEN_SIZE          OSAL_IOR(IOC_TYPE_GFBG, 131, ot_fb_size)
/* To display multiple surfaces in turn and set the alpha and colorkey attributes */
#define FBIOFLIP_SURFACE            OSAL_IOW(IOC_TYPE_GFBG, 132, ot_fb_surfaceex)
/* To set the compression function status of an overlay layer */
#define FBIOPUT_COMPRESSION_GFBG    OSAL_IOW(IOC_TYPE_GFBG, 133, td_bool)
/* To obtain the compression function status of an overlay layer */
#define FBIOGET_COMPRESSION_GFBG    OSAL_IOR(IOC_TYPE_GFBG, 134, td_bool)
/* To set the hdr function status of an overlay layer */
#define FBIOPUT_DYNAMIC_RANGE_GFBG  OSAL_IOW(IOC_TYPE_GFBG, 139, ot_fb_dynamic_range)
/* To get the hdr function status of an overlay layer */
#define FBIOGET_DYNAMIC_RANGE_GFBG  OSAL_IOR(IOC_TYPE_GFBG, 140, ot_fb_dynamic_range)
/* To create the layer */
#define FBIO_CREATE_LAYER           OSAL_IO(IOC_TYPE_GFBG, 149)
/* To destroy the layer */
#define FBIO_DESTROY_LAYER          OSAL_IO(IOC_TYPE_GFBG, 150)

/* cursor handle */
/* Attention:surface in cursor will be released by user */
#define FBIOPUT_CURSOR_INFO           OSAL_IOW(IOC_TYPE_GFBG, 104, ot_fb_cursor)
#define FBIOGET_CURSOR_INFO           OSAL_IOW(IOC_TYPE_GFBG, 105, ot_fb_cursor)

#define FBIOPUT_CURSOR_STATE          OSAL_IOW(IOC_TYPE_GFBG, 106, td_bool)
#define FBIOGET_CURSOR_STATE          OSAL_IOW(IOC_TYPE_GFBG, 107, td_bool)

#define FBIOPUT_CURSOR_POS            OSAL_IOW(IOC_TYPE_GFBG, 108, ot_fb_point)
#define FBIOGET_CURSOR_POS            OSAL_IOR(IOC_TYPE_GFBG, 109, ot_fb_point)

#define FBIOPUT_CURSOR_COLORKEY       OSAL_IOR(IOC_TYPE_GFBG, 110, ot_fb_colorkey)
#define FBIOGET_CURSOR_COLORKEY       OSAL_IOW(IOC_TYPE_GFBG, 111, ot_fb_colorkey)
#define FBIOPUT_CURSOR_ALPHA          OSAL_IOR(IOC_TYPE_GFBG, 112, ot_fb_alpha)
#define FBIOGET_CURSOR_ALPHA          OSAL_IOW(IOC_TYPE_GFBG, 113, ot_fb_alpha)
...
#define FBIOPUT_CURSOR_ATTCH_CURSOR    OSAL_IOW(IOC_TYPE_GFBG, 114, td_u32)
#define FBIOPUT_CURSOR_DETACH_CURSOR   OSAL_IOW(IOC_TYPE_GFBG, 115, td_u32)
/* To set the layer information */
#define FBIOPUT_LAYER_INFO                OSAL_IOW(IOC_TYPE_GFBG, 120, ot_fb_layer_info)
/* To get the layer information */
#define FBIOGET_LAYER_INFO                OSAL_IOR(IOC_TYPE_GFBG, 121, ot_fb_layer_info)
/* To get canvas buf */
#define FBIOGET_CANVAS_BUF             OSAL_IOR(IOC_TYPE_GFBG, 123, ot_fb_buf)
/* To refresh the displayed contents in extended mode */
#define FBIO_REFRESH                      OSAL_IOW(IOC_TYPE_GFBG, 124, ot_fb_buf)
/* sync refresh */
#define FBIO_WAITFOR_FREFRESH_DONE        OSAL_IO(IOC_TYPE_GFBG, 125)
/* To set the mirror mode */
#define FBIOPUT_MIRROR_MODE            OSAL_IOW(IOC_TYPE_GFBG, 126, ot_fb_mirror_mode)
/* To get the mirror mode */
#define FBIOGET_MIRROR_MODE            OSAL_IOW(IOC_TYPE_GFBG, 127, ot_fb_mirror_mode)
/* To set the rotate mode */
#define FBIOPUT_ROTATE_MODE            OSAL_IOW(IOC_TYPE_GFBG, 128, ot_fb_rotate_mode)
/* To get the rotate mode */
#define FBIOGET_ROTATE_MODE            OSAL_IOW(IOC_TYPE_GFBG, 129, ot_fb_rotate_mode)
/* To set the DDR detect zone of an overlay layer */
#define FBIOPUT_MDDRDETECT_GFBG    OSAL_IOW(IOC_TYPE_GFBG, 135, ot_fb_ddr_zone)
/* To get the DDR detect zone of an overlay layer */
#define FBIOGET_MDDRDETECT_GFBG    OSAL_IOW(IOC_TYPE_GFBG, 136, ot_fb_ddr_zone)
/* To draw smart rect */
#define FBIO_DRAW_SMART_RECT    OSAL_IOW(IOC_TYPE_GFBG, 151, ot_fb_smart_rect_param)
