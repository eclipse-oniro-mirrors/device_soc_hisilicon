...
#define OT_OSAL_WAIT_DEFAULT_MS 100
...
/* completion api */
#define OSAL_HZ    100
typedef struct osal_completion {
    void *comp;
} osal_completion_t;
void osal_init_completion(struct osal_completion *comp);
int osal_wait_for_completion_timeout(struct osal_completion *comp, long timeout_ms);
void osal_complete(struct osal_completion *comp);
...
static inline int osal_wait_event_interruptible_impl(osal_wait_t *wait,
    osal_wait_cond_func_t func, const void *param)
{
    int ret = 0;
    while (func(param) == 0) {
        ret = osal_wait_timeout_interruptible(wait, func, param, OT_OSAL_WAIT_DEFAULT_MS);
        if (ret < 0) {
            break;
        }
    }
...
static inline int osal_wait_event_uninterruptible_impl(osal_wait_t *wait,
    osal_wait_cond_func_t func, const void *param)
{
    int ret = 0;
    while (func(param) == 0) {
        ret = osal_wait_uninterruptible(wait, func, param);
        if (ret < 0) {
            break;
        }
    }
...
static int osal_wait_event_timeout_interruptible_impl(osal_wait_t *wait,
    osal_wait_cond_func_t func, const void *param, int timeout)
{
    int ret = timeout;
    ...
    while (func(param) == 0) {
        ret = osal_wait_timeout_interruptible(wait, func, param, ret);
        if ((ret == 0) || (ret == -ERESTARTSYS)) {
            return ret;
        }
    }
    return (int)osal_msecs_to_jiffies(ret);
}

static int osal_wait_event_timeout_uninterruptible_impl(osal_wait_t *wait,
    osal_wait_cond_func_t func, const void *param, int timeout)
{
    int ret = timeout;
    if ((func(param) != 0) && (timeout == 0)) {
...
#define osal_wait_event_interruptible(wait, func, param) \
    osal_wait_event_interruptible_impl((wait), (func), (param))
#define osal_wait_event_uninterruptible(wait, func, param) \
    osal_wait_event_uninterruptible_impl((wait), (func), (param))
#define osal_wait_event_timeout_interruptible(wait, func, param, timeout) \
    osal_wait_event_timeout_interruptible_impl((wait), (func), (param), (timeout))
#define osal_wait_event_timeout_uninterruptible(wait, func, param, timeout) \
    osal_wait_event_timeout_uninterruptible_impl((wait), (func), (param), (timeout))
...
#define osal_abs(x) ({ __typeof__(x) __x = (x); (__x < 0) ? -__x : __x; })
...
int osal_registerdevice(osal_dev_t *pdev);
...
#define osal_module_driver(osal_driver, osal_register, osal_unregister, ...)  \
    static int __init osal_driver##_init(void) { return osal_register(&(osal_driver)); } \
    module_init(osal_driver##_init);                                          \
    static void __exit osal_driver##_exit(void) { osal_unregister(&(osal_driver)); } \
