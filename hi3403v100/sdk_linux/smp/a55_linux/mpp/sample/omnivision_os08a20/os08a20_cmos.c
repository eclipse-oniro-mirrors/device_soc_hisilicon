...
static inline td_u8 high_8bits(td_u32 x)
{
    return (td_u8)((x & 0xff00) >> BYTE_SHIFT);
}
static inline td_u8 low_8bits(td_u32 x)
{
    return (td_u8)(x & 0x00ff);
}

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/
static inline td_float os08a20_set_sensor_acc(td_float static_val, td_float src)
{
    return (static_val > 0) ? static_val : src;
}
...
static inline td_void os08a20_sensor_set_ctx(ot_vi_pipe dev, ot_isp_sns_state *ctx)
{
    g_os08a20_sns_state[dev] = ctx;
}

static inline td_void os08a20_sensor_reset_ctx(ot_vi_pipe dev)
{
    g_os08a20_sns_state[dev] = TD_NULL;
}
...
const Os08a20VideoModeTbl OS08A20_MODE_TBL[OS08A20_MODE_BUTT] = {
...
#define OS08A20_DGAIN_ACCURACY       (1.0 / 128)
#define OS08A20_EXPO_LINE_MARGIN     10
#define OS08A20_LONG_FRAME_INT_STEP  90


static td_void os08a20_err_mode_print(const ot_isp_cmos_sensor_image_mode *sensor_image_mode,
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->int_time_accu.accuracy = os08a20_set_sensor_acc(g_init_int_time_acc[vi_pipe], 1);

    ae_sns_dft->again_accu.accu_type = OT_ISP_AE_ACCURACY_TABLE;
    ae_sns_dft->again_accu.accuracy = os08a20_set_sensor_acc(g_init_again_acc[vi_pipe], 1);

    ae_sns_dft->dgain_accu.accu_type = OT_ISP_AE_ACCURACY_LINEAR;
    ae_sns_dft->dgain_accu.accuracy = os08a20_set_sensor_acc(g_init_dgain_acc[vi_pipe], OS08A20_DGAIN_ACCURACY);
...
static td_s32 cmos_get_ae_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    sns_state = os08a20_sensor_get_ctx(vi_pipe);
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    lines = OS08A20_MODE_TBL[sns_state->img_mode].ver_lines;
    lines_max = OS08A20_MODE_TBL[sns_state->img_mode].max_ver_lines;
    max_fps = OS08A20_MODE_TBL[sns_state->img_mode].max_fps;
    min_fps = OS08A20_MODE_TBL[sns_state->img_mode].min_fps;
    ...
    ae_sns_dft->fps = (achieve_fps_ok == TD_TRUE) ? fps : ((vmax > 0) ? (lines * max_fps / vmax) : max_fps);
    ae_sns_dft->full_lines_std = sns_state->fl_std;
    ae_sns_dft->max_int_time = sns_state->fl_std - 10; /* sub 10 */
...
static td_void cmos_step_limit(ot_isp_ae_int_time_range *int_time,
    const TimeStep *step, td_u32 full_lines, td_u32 wdr_frame_num)
{
    td_u32 i;
...
static td_void cmos_get_inttime_max_2to1_long_frame(ot_vi_pipe vi_pipe, ot_isp_ae_int_time_range *int_time,
    td_u32 short_time_min_limit, const ot_isp_sns_state *sns_state, const TimeStep *step)
{
    td_u32 short_max0;
    td_u32 short_max;
    ...
    cmos_set_2to1_long_frame_range(int_time, short_time_min_limit, short_max, OS08A20_LONG_FRAME_INT_STEP);
}
                                              ...
                                              ot_isp_ae_int_time_range *int_time, td_u32 *lf_max_int_time)
{
    ...
    for (i = 0; i < 2; ++i) { /* frame number is 2 */
        step.inc[i] = (sns_state->fl[1] - OS08A20_MODE_TBL[OS08A20_8M_30FPS_10BIT_2TO1_VC_MODE].height) >> 1;
        step.dec[i] = (sns_state->fl[1] - OS08A20_MODE_TBL[OS08A20_8M_30FPS_10BIT_2TO1_VC_MODE].height) >> 1;
    }

    if (g_fswdr_mode[vi_pipe] == OT_ISP_FSWDR_LONG_FRAME_MODE) {
        cmos_get_inttime_max_2to1_long_frame(vi_pipe, int_time, short_time_min_limit, sns_state, &step);
    } else {
...
static td_void cmos_set_pixel_detect(ot_vi_pipe vi_pipe, td_bool enable)
{
    ...
    if (enable) { /* setup for ISP pixel calibration mode */
        ...
        os08a20_write_register(vi_pipe, OS08A20_EXPO_L_ADDR, low_8bits(full_lines_5fps - OS08A20_EXPO_LINE_MARGIN));
        os08a20_write_register(vi_pipe, OS08A20_EXPO_H_ADDR, high_8bits(full_lines_5fps - OS08A20_EXPO_LINE_MARGIN));
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sensor_image_mode *sensor_image_mode)
{
    ...
    for (i = 0; i < OS08A20_MODE_BUTT; i++) {
        if (sensor_image_mode->fps <= OS08A20_MODE_TBL[i].max_fps &&
            sensor_image_mode->width <= OS08A20_MODE_TBL[i].width &&
            sensor_image_mode->height <= OS08A20_MODE_TBL[i].height &&
            sns_state->wdr_mode == OS08A20_MODE_TBL[i].wdr_mode) {
            image_mode = (Os08a20ResMode)i;
            break;
        }
    }
...
static td_s32 sensor_ctx_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state_ctx = os08a20_sensor_get_ctx(vi_pipe);
...
ot_isp_sns_obj *os08a20_get_sns_obj(void)
{
