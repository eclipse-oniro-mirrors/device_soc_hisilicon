...
{
    int ret;
    char devFile[I2C_DEV_FILE_NUM] = {0};
    td_u8 dev_num;
    ...
    (td_void)snprintf_s(devFile, sizeof(devFile), sizeof(devFile) - 1, "/dev/i2c-%u", dev_num);

    g_fd[vi_pipe] = open(devFile, O_RDWR, S_IRUSR | S_IWUSR);
    if (g_fd[vi_pipe] < 0) {
