...
#include "hy_s0603_cmos_ex.h"
#include "hy_s0603_cmos.h"

#define HY_S0603_ID                    603
#define SENSOR_HY_S0603_WIDTH          3840
#define SENSOR_HY_S0603_HEIGHT         2160
 ...
 * global variables                                                            *
 ****************************************************************************/
#define hy_s0603_sensor_set_ctx(dev, ctx)   ((g_hy_s0603_sns_state[dev]) = (ctx))
#define hy_s0603_sensor_reset_ctx(dev)      (g_hy_s0603_sns_state[dev] = TD_NULL)
...
static td_u32 g_max_short_exp[OT_ISP_MAX_PIPE_NUM] = {[0 ...(OT_ISP_MAX_PIPE_NUM - 1)] = HY_S0603_SEXP_MAX_DEFAULT};

ot_isp_sns_commbus g_hy_s0603_bus_info[OT_ISP_MAX_PIPE_NUM] = {
    [0] = { .i2c_dev = 0},
...
ot_isp_sns_state *g_hy_s0603_sns_state[OT_ISP_MAX_PIPE_NUM] = {TD_NULL};
...
{
    return &g_hy_s0603_bus_info[vi_pipe];
}

ot_isp_sns_state *hy_s0603_get_ctx(ot_vi_pipe vi_pipe)
{
    return g_hy_s0603_sns_state[vi_pipe];
}


const hy_s0603_video_mode_tbl g_hy_s0603_mode_tbl[HY_S0603_MODE_BUTT] = {
    {HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR,   HY_S0603_FULL_LINES_MAX,          30, 2.5,
     3840, 2160, 0, OT_WDR_MODE_NONE,       "HY_S0603_8M_30FPS_12BIT_LINEAR_MODE"},
    {HY_S0603_VMAX_8M_30FPS_10BIT_2TO1_WDR,  HY_S0603_FULL_LINES_MAX_2TO1_WDR, 30, 10.0,
     3840, 2160, 0, OT_WDR_MODE_2To1_LINE,  "HY_S0603_8M_30FPS_10BIT_2TO1_VC_MODE"},
};


#define hy_s0603_err_mode_print(sns_image_mode, sns_state) \
do { \
...
static td_s32 cmos_get_ae_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    hy_s0603_sensor_get_ctx(vi_pipe, sns_state);
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    lines = g_hy_s0603_mode_tbl[sns_state->img_mode].ver_lines;
    lines_max = g_hy_s0603_mode_tbl[sns_state->img_mode].max_ver_lines;
    max_fps = g_hy_s0603_mode_tbl[sns_state->img_mode].max_fps;
    min_fps = g_hy_s0603_mode_tbl[sns_state->img_mode].min_fps;

    if ((fps > max_fps) || (fps < min_fps)) {
        isp_err_trace("ISP sensor hy_s0603 Not support Fps: %f\n", fps);
        return;
    }
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_NONE) {
        ...
        g_max_short_exp[vi_pipe] = (td_u32)(HY_S0603_SEXP_MAX_DEFAULT * max_fps / div_0_to_1_float(fps));
        ae_sns_dft->max_int_time = sns_state->fl_std - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG;
...
#define HY_S0603_AGAIN_NODE_NUM               256
#define HY_S0603_AGAIN_ADDR_INDEX_NODE_NUM    256
#define HY_S0603_DGAIN_NODE_NUM               256
#define HY_S0603_DGAIN_ADDR_INDEX_NODE_NUM    256

static td_u32 again_index[HY_S0603_AGAIN_NODE_NUM] = {
...
static td_u32 dgain_index[HY_S0603_DGAIN_NODE_NUM] = {
    1024, 1040, 1056, 1072, 1088, 1104, 1120, 1136, 1152, 1168, 1184, 1200, 1216, 1232, 1248, 1264,
    1280, 1296, 1312, 1328, 1344, 1360, 1376, 1392, 1408, 1424, 1440, 1456, 1472, 1488, 1504, 1520,
    1536, 1552, 1568, 1584, 1600, 1616, 1632, 1648, 1664, 1680, 1696, 1712, 1728, 1744, 1760, 1776,
    1792, 1808, 1824, 1840, 1856, 1872, 1888, 1904, 1920, 1936, 1952, 1968, 1984, 2000, 2016, 2032,
...
static td_u32 again_table[HY_S0603_AGAIN_ADDR_INDEX_NODE_NUM] = {
...
static td_u32 dgain_table[HY_S0603_DGAIN_ADDR_INDEX_NODE_NUM] = {
    ...
    0x00D4, 0x00D6, (0x00D0 + 0x08), 0x00DA, 0x00DC, 0x00DE, 0x00E0, 0x00E2, 0x00E4, 0x00E6,
    0x00E8, 0x00EA, 0x00EC, 0x00EE,
    ...
    (0x01D0 + 0x08), 0x01DA, 0x01DC, 0x01DE,
    ...
    0x03D0, 0x03D2, 0x03D4, 0x03D6, (0x03D0 + 0x08), 0x03DA, 0x03DC, 0x03DE, 0x03E0, 0x03E2,
    0x03E4, 0x03E6, 0x03E8, 0x03EA,
    ...
    0x07C0, 0x07C2, 0x07C4, 0x07C6, 0x07C8, 0x07CA, 0x07CC, 0x07CE, 0x07D0, 0x07D2,
    0x07D4, 0x07D6, (0x07D0 + 0x08), 0x07DA,
...
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *again_lin, td_u32 *again_db)
{
    ...
    if (*again_lin >= (td_u32)(again_index[HY_S0603_AGAIN_ADDR_INDEX_NODE_NUM - 1] * g_dcg_ratio)) {
        *again_lin = (td_u32)(again_index[HY_S0603_AGAIN_ADDR_INDEX_NODE_NUM - 1] * g_dcg_ratio);
        if (g_dcg_ratio > 1) {
            *again_db = again_table[HY_S0603_AGAIN_NODE_NUM - 1] | 0x2000;
        } else {
            *again_db = again_table[HY_S0603_AGAIN_NODE_NUM - 1];
        }
        return;
    }
    if (*again_lin > (1024 * g_dcg_ratio)) {  /* 1024 */
        again_lin_tmp = (td_u32)(*again_lin / g_dcg_ratio);
        for (i = 1; i < HY_S0603_AGAIN_NODE_NUM; i++) {
...
static td_void cmos_dgain_calc_table(ot_vi_pipe vi_pipe, td_u32 *dgain_lin, td_u32 *dgain_db)
{
    ...
    if (*dgain_lin >= dgain_index[HY_S0603_DGAIN_ADDR_INDEX_NODE_NUM - 1]) {
        *dgain_lin = dgain_index[HY_S0603_DGAIN_ADDR_INDEX_NODE_NUM - 1];
        *dgain_db = dgain_table[HY_S0603_DGAIN_NODE_NUM - 1];
        return;
    }

    for (i = 1; i < HY_S0603_DGAIN_NODE_NUM; i++) {
...
static td_s32 cmos_get_isp_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sensor_mode.sensor_id = HY_S0603_ID;
...
static td_void cmos_default_logic_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].i2c_data[SNS_LOGIC_REG363C].reg_addr = HY_S0603_LOGIC_ADDR_2;
    return;
}
...
static td_void cmos_comm_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].com_bus.i2c_dev = g_hy_s0603_bus_info[vi_pipe].i2c_dev;
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = HY_S0603_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = HY_S0603_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = HY_S0603_DATA_BYTE;
    }

    /* Linear Mode Regs */
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].reg_addr = HY_S0603_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].reg_addr = HY_S0603_EXPO_M_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].reg_addr = HY_S0603_EXPO_H_ADDR;

    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].reg_addr = HY_S0603_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_H_IDX].reg_addr = HY_S0603_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].reg_addr = HY_S0603_DGAIN_H_ADDR;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].reg_addr = HY_S0603_DGAIN_L_ADDR;

    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].reg_addr = HY_S0603_VMAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].reg_addr = HY_S0603_VMAX_H_ADDR;
...
static td_void cmos_2to1_line_wdr_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_L_IDX].reg_addr = HY_S0603_SHORT_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_M_IDX].reg_addr = HY_S0603_SHORT_EXPO_M_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].reg_addr = HY_S0603_SHORT_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].reg_addr = HY_S0603_SHORT_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].reg_addr = HY_S0603_SHORT_DGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].reg_addr = HY_S0603_SHORT_DGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_L_ADDR_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_L_ADDR_IDX].reg_addr = HY_S0603_SHORT_EXP_MAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_H_ADDR_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_H_ADDR_IDX].reg_addr = HY_S0603_SHORT_EXP_MAX_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_LOGIC_L_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[WDR_LOGIC_L_IDX].reg_addr = HY_S0603_WDR_LOGIC_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_LOGIC_H_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[WDR_LOGIC_H_IDX].reg_addr = HY_S0603_WDR_LOGIC_H_ADDR;
    return;
}
    ...
    ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case HY_S0603_8M_30FPS_12BIT_LINEAR_MODE:
            sns_state->fl_std = HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR;
            break;
        case HY_S0603_8M_30FPS_10BIT_2TO1_VC_MODE:
            sns_state->fl_std = HY_S0603_VMAX_8M_30FPS_10BIT_2TO1_WDR;
            break;
        default:
            sns_state->fl_std = HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR;
            break;
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sensor_image_mode *sns_image_mode)
{
    td_u32 i;
    td_u8 image_mode;
    ot_isp_sns_state *sns_state = TD_NULL;
    sensor_check_pointer_return(sns_image_mode);
    hy_s0603_sensor_get_ctx(vi_pipe, sns_state);
    sensor_check_pointer_return(sns_state);

    image_mode = sns_state->img_mode;

    for (i = 0; i < HY_S0603_MODE_BUTT; i++) {
        if (sns_image_mode->fps <= g_hy_s0603_mode_tbl[i].max_fps &&
            sns_image_mode->width <= g_hy_s0603_mode_tbl[i].width &&
            sns_image_mode->height <= g_hy_s0603_mode_tbl[i].height &&
            sns_state->wdr_mode == g_hy_s0603_mode_tbl[i].wdr_mode) {
            image_mode = (hy_s0603_res_mode)i;
            break;
        }
    }

    if (i >= HY_S0603_MODE_BUTT) {
        hy_s0603_err_mode_print(sns_image_mode, sns_state);
        return TD_FAILURE;
    }
...
static td_void sensor_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode  = HY_S0603_8M_30FPS_12BIT_LINEAR_MODE;
    sns_state->wdr_mode  = OT_WDR_MODE_NONE;
    sns_state->fl_std    = HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR;
    sns_state->fl[0]     = HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR;
    sns_state->fl[1]     = HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR;
...
static td_void cmos_set_pixel_detect(ot_vi_pipe vi_pipe, td_bool enable)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        ...
        if (sns_state->img_mode == HY_S0603_8M_30FPS_12BIT_LINEAR_MODE) {
            full_lines_5fps = (HY_S0603_VMAX_8M_30FPS_12BIT_LINEAR * 30) / 5; /* 30fps, 5fps */
        } else {
            return;
        }
    }
    ...
    if (enable) { /* setup for ISP pixel calibration mode */
        hy_s0603_write_register(vi_pipe, HY_S0603_AGAIN_L_ADDR, 0x40);
        hy_s0603_write_register(vi_pipe, HY_S0603_AGAIN_H_ADDR, 0x03);

        hy_s0603_write_register(vi_pipe, HY_S0603_DGAIN_H_ADDR, 0x00);
        hy_s0603_write_register(vi_pipe, HY_S0603_DGAIN_L_ADDR, 0x80);

        hy_s0603_write_register(vi_pipe, HY_S0603_VMAX_L_ADDR, low_8bits(full_lines_5fps));
        hy_s0603_write_register(vi_pipe, HY_S0603_VMAX_H_ADDR, high_8bits(full_lines_5fps));

        hy_s0603_write_register(vi_pipe, HY_S0603_EXPO_L_ADDR, lower_4bits(full_lines_5fps - 10));
        hy_s0603_write_register(vi_pipe, HY_S0603_EXPO_M_ADDR, higher_8bits(full_lines_5fps - 10));
        hy_s0603_write_register(vi_pipe, HY_S0603_EXPO_H_ADDR, higher_4bits(full_lines_5fps - 10));
    } else { /* setup for ISP 'normal mode' */
        sns_state->fl_std = (sns_state->fl_std > HY_S0603_FULL_LINES_MAX) ? HY_S0603_FULL_LINES_MAX : sns_state->fl_std;
        hy_s0603_write_register(vi_pipe, HY_S0603_VMAX_L_ADDR, low_8bits(sns_state->fl_std));
        hy_s0603_write_register(vi_pipe, HY_S0603_VMAX_H_ADDR, high_8bits(sns_state->fl_std));
        sns_state->sync_init = TD_FALSE;
    }
...
static td_s32 cmos_init_sensor_exp_function(ot_isp_sensor_exp_func *sensor_exp_func)
{
    ...
    sensor_exp_func->pfn_cmos_sensor_init         = hy_s0603_init;
    sensor_exp_func->pfn_cmos_sensor_exit         = hy_s0603_exit;
...
static td_s32 hy_s0603_set_bus_info(ot_vi_pipe vi_pipe, ot_isp_sns_commbus sns_bus_info)
{
    g_hy_s0603_bus_info[vi_pipe].i2c_dev = sns_bus_info.i2c_dev;
...
static td_s32 sensor_ctx_init(ot_vi_pipe vi_pipe)
{
    ...
    hy_s0603_sensor_get_ctx(vi_pipe, sns_state_ctx);
    ...
    hy_s0603_sensor_set_ctx(vi_pipe, sns_state_ctx);
...
static td_void sensor_ctx_exit(ot_vi_pipe vi_pipe)
{
    ...
    hy_s0603_sensor_reset_ctx(vi_pipe);
...
static td_s32 sensor_register_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    sns_attr_info.sensor_id = HY_S0603_ID;
...
static td_s32 sensor_unregister_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    ret = ot_mpi_isp_sensor_unreg_callback(vi_pipe, HY_S0603_ID);
    if (ret != TD_SUCCESS) {
        isp_err_trace("sensor unregister callback function failed!\n");
        return ret;
    }

    ret = ot_mpi_ae_sensor_unreg_callback(vi_pipe, ae_lib, HY_S0603_ID);
    if (ret != TD_SUCCESS) {
        isp_err_trace("sensor unregister callback function to ae lib failed!\n");
        return ret;
    }

    ret = ot_mpi_awb_sensor_unreg_callback(vi_pipe, awb_lib, HY_S0603_ID);
    if (ret != TD_SUCCESS) {
...
{
    hy_s0603_set_blc_clamp_value(vi_pipe, blc_clamp.blc_clamp_en);

    /* After enabling blc_clamp_en in hy_s0603, the internal black level of the sensor will take effect in the OB area.

       Regardless of whether blc_clamp_en is enabled or disabled,
       the black level in the OB area will be the same as that in the visible region.

       This means that enabling blc_clamp_en does not affect the use of dynamic black level.

       Since hy_s0603 has a linear black level of 9000 and a WDR black level of 16000 when blc_clamp_en is disabled,
       the excessively high black level can affect the dynamic range of the image, resulting in a smaller dynamic range.

       Considering both functionality and visual effects, the blc_clamp_en register is fixed to true,
       while the enablement interface remains normal for acquisition and configuration. */
}

ot_isp_sns_obj g_sns_hy_s0603_obj = {
    .pfn_register_callback     = sensor_register_callback,
    .pfn_un_register_callback  = sensor_unregister_callback,
    .pfn_standby               = hy_s0603_standby,
    .pfn_restart               = hy_s0603_restart,
    .pfn_mirror_flip           = TD_NULL,
    .pfn_set_blc_clamp         = hy_s0603_blc_clamp,
    .pfn_write_reg             = hy_s0603_write_register,
    .pfn_read_reg              = hy_s0603_read_register,
    .pfn_set_bus_info          = hy_s0603_set_bus_info,
    .pfn_set_init              = sensor_set_init
};

ot_isp_sns_obj *hy_s0603_get_sns_obj(void)
{
